            ...
            if (mcdu.flightPlanManager.getDestination() && mcdu.flightPlanManager.getDestination().ident) {
                ...
                mcdu.onLeftInput[1] = async (value, scratchpadCallback) => {
                    ...
                    case 'NONE':
                        ...
                        } else if (await mcdu.tryUpdateAltDestination(value)) {
                            CDUInitPage.ShowPage1(mcdu);
                        } else {
                            scratchpadCallback();
                        }
    ...
    static ShowPage2(mcdu) {
        ...
        mcdu.onRightInput[0] = async (value, scratchpadCallback) => {
            if (value === '') {
                let zfw = undefined;
                let zfwCg = undefined;
                const aircraftBoarding = SimVar.GetSimVarValue('L:A32NX_BOARDING_STARTED_BY_USR', 'bool');
        ...
        mcdu.onRightInput[1] = async (value, scratchpadCallback) => {
            ...
            } else if (await mcdu.trySetBlockFuel(value)) {
                CDUInitPage.updateTowIfNeeded(mcdu);
                CDUInitPage.ShowPage2(mcdu);
            } else {
                scratchpadCallback();
        ...
        mcdu.onLeftInput[0] = async (value, scratchpadCallback) => {
            ...
            } else if (mcdu.trySetTaxiFuelWeight(value)) {
                CDUInitPage.updateTowIfNeeded(mcdu);
                CDUInitPage.ShowPage2(mcdu);
            } else {
                scratchpadCallback();
