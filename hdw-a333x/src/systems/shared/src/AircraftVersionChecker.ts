...
/**
 * Contains the a333x_build_info.json file's information in a structured way.
 */
...
export class AircraftVersionChecker {
  ...
  /**
   * Reads the a333x_build_info.json file and returns the data a BuildInfo object.
  ...
  private static async initialize() {
    this.releaseInfo = await GitVersions.getReleases('headwindsim', 'a339x', false, 0, 1);
    this.newestCommit = await GitVersions.getNewestCommit('headwindsim', 'a339x', 'main');
    this.newestExpCommit = await GitVersions.getNewestCommit('headwindsim', 'a339x', 'experimental');
