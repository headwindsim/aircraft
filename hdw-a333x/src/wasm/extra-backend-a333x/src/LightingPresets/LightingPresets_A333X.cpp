...
#include "LightingPresets_A333X.h"
...
// The LightingPresets_A333X module uses the DataManager to get and set variables.
...
// LightingPresets_A333X will be dormant most of the time, this is saving a lot of
// unnecessary reads/writes.
///

bool LightingPresets_A333X::initialize_aircraft() {
  ...
  _isInitialized = true;
  LOG_INFO("LightingPresets_A333X initialized");
  return true;
}
...
void LightingPresets_A333X::readFromAircraft() {
...
void LightingPresets_A333X::applyToAircraft() {
...
void LightingPresets_A333X::loadFromIni(const mINI::INIStructure& ini, const std::string& iniSectionName) {
...
void LightingPresets_A333X::saveToIni(mINI::INIStructure& ini, const std::string& iniSectionName) const {
...
[[maybe_unused]] std::string LightingPresets_A333X::str() const {
...
void LightingPresets_A333X::setValidCabinLightValue(FLOAT64 level) {
  ...
  // cabin lights in the A333X need to be controlled by two vars
...
bool LightingPresets_A333X::calculateIntermediateValues(FLOAT64 stepSize) {
  // clang-format off
