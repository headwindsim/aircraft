...
function canInitiateTO(_fmc) {
    return SimVar.GetSimVarValue("CAMERA STATE", "number") < 10 && Simplane.getAltitudeAboveGround() > 7.5 ||
    (
...
class A32NX_FlightPhase_TakeOff {
    ...
    check(_deltaTime, _fmc) {
        const isAcOnGround = Simplane.getAltitudeAboveGround() <= 7.5;
...
class A32NX_FlightPhase_Climb {
    ...
    check(_deltaTime, _fmc) {
        if (!_fmc.isAnEngineOn() && Simplane.getAltitudeAboveGround() < 7.5) {
            this.nextFmgcFlightPhase = FmgcFlightPhases.DONE;
            return true;
        }
...
class A32NX_FlightPhase_Cruise {
    ...
    check(_deltaTime, _fmc) {
        return !_fmc.isAnEngineOn() && Simplane.getAltitudeAboveGround() < 7.5;
    }
}
...
class A32NX_FlightPhase_Approach {
    ...
    check(_deltaTime, _fmc) {
        ...
        return this.landingConfirmation.write(Simplane.getAltitudeAboveGround() < 7.5, _deltaTime) || !_fmc.isAnEngineOn();
    }
}
