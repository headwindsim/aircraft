...
export class NXDataStore {
  ...
  static get(key: string, defaultVal?: string): any {
    const val = GetStoredData(`SU95X_${key}`);
  ...
  static set(key: string, val: string): void {
    SetStoredData(`SU95X_${key}`, val);
    this.listener.triggerToAllSubscribers('SU95X_NXDATASTORE_UPDATE', key, val);
  }

  static subscribe(key: string, callback: SubscribeCallback): SubscribeCancellation {
    return Coherent.on('SU95X_NXDATASTORE_UPDATE', (updatedKey: string, value: string) => {
