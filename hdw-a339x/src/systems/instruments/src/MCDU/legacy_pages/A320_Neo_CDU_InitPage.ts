  ...
  static ShowPage1(mcdu: LegacyFmsPageInterface) {
    ...
    if (origin) {
      if (dest) {
        ...
        // Pax number
        [paxNbrAction, paxNbrText, paxNbrColor] = new CDU_SingleValueField(
          mcdu,
          'int',
          mcdu.isPaxNbrSet ? mcdu.paxNbr : null,
          {
            clearable: true,
          ...
          (value) => {
            if (typeof value === 'number') {
              mcdu.paxNbr = value;
              // mcdu.isPaxNbrSet = true;
            } else {
              // mcdu.isPaxNbrSet = false;
              mcdu.paxNbr = undefined;
            }
            CDUInitPage.ShowPage1(mcdu);
          },
        ).getFieldAsColumnParameters();

        mcdu.onLeftInput[3] = paxNbrAction;
    ...
    mcdu.setTemplate(
      FormatTemplate([
        ...
        [new Column(0, paxNbrText, paxNbrColor), new Column(23, 'WIND/TEMP>', Column.right)],
