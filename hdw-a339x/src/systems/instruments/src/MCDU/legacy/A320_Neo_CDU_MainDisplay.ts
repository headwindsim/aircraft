  ...
  implements LegacyFmsPageInterface, LegacyAtsuPageInterface
{
  ...
  private scratchpads?: Record<'MCDU' | 'FMGC' | 'ATSU' | 'ACARS' | 'ACMS' | 'CMS' | 'SAT', ScratchpadDataLink>;
  ...
  private _activeSystem: 'FMGC' | 'ATSU' | 'ACARS' | 'ACMS' | 'CMS' | 'SAT' = 'FMGC';
    ...
    switch (event.type) {
      case 'open': {
        ...
        new NXNotifManager().showNotification({
          ...
          message: 'A339X MCDU successfully connected to SimBridge MCDU Server.',
          timeout: 5000,
        });
  ...
  protected Init() {
    ...
    this.scratchpads = {
      ...
      ACARS: new ScratchpadDataLink(this, this.scratchpadDisplay, 'ACARS'),
   ...
   */
  private updateAnnunciatorsForSide(side: 'left' | 'right' | 'center', lightTest: boolean, powerOn: boolean, forceWrite = false) {
    let updateNeeded = false;
    ...
    for (const [annunc, state] of Object.entries(states)) {
      ...
      if (annunc === 'fmgc') {
        ...
        newState =
          newState ||
          this.isSubsystemRequesting('ACARS') ||
          this.isSubsystemRequesting('ACMS') ||
          this.isSubsystemRequesting('CMS') ||
          this.isSubsystemRequesting('ATSU') ||
          this.isSubsystemRequesting('SAT');
      }
   ...
   */
  public set activeSystem(subsystem: 'ACARS' | 'ACMS' | 'CMS' | 'SAT' | 'ATSU' | 'FMGC') {
  ...
  public get acarsScratchpad() {
    return this.scratchpads['ACARS'];
  }

  public get acmsScratchpad() {
    return this.scratchpads['ACMS'];
  }

  public get cmsScratchpad() {
    return this.scratchpads['CMS'];
  }

  public get satScratchpad() {
   ...
   */
  public isSubsystemRequesting(subsystem: 'ACARS' | 'ACMS' | 'CMS' | 'SAT' | 'ATSU' | 'FMGC') {
   ...
   */
  public setRequest(subsystem: 'ACARS' | 'ACMS' | 'CMS' | 'SAT' | 'ATSU' | 'FMGC') {
   ...
   */
  private _clearRequest(subsystem: 'ACARS' | 'ACMS' | 'CMS' | 'SAT' | 'ATSU' | 'FMGC') {
  ...
  private initKeyboardScratchpad() {
    window.document.addEventListener('click', () => {
      ...
      const isPoweredC = SimVar.GetSimVarValue('L:A32NX_ELEC_AC_2_BUS_IS_POWERED', 'Number');

      // TODO: L/R/C MCDU
      if (mcduInput === 'ENABLED') {
        this.inFocus = !this.inFocus;
        if (this.inFocus && (isPoweredL || isPoweredR || isPoweredC)) {
          // This is legal but the TS DOM types mark it readonly.
   ...
   */
  public onBrightnessKey(side: 'L' | 'R' | 'C', sign: -1 | 1) {
    let oldBrightness;
  ...
  public sendUpdate() {
    ...
    if (mcdu3Powered) {
      ...
      center.displayBrightness = this.centerBrightness / A320_Neo_CDU_MainDisplay.MAX_BRIGHTNESS;
    }
  ...
  private sendClearScreen() {
    ...
    const right = left;
    const center = left;
    const content = { right, left, center };
    this.sendToMcduServerClient(`update:${JSON.stringify(content)}`);
  }
