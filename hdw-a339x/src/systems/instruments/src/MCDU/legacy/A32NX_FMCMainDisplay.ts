...
import { A330AircraftConfig } from '@fmgc/flightplanning/A330AircraftConfig';
...
import { CDUIdentPage } from '../legacy_pages/A330_Neo_CDU_IdentPage';
...
export abstract class FMCMainDisplay implements FmsDataInterface, FmsDisplayInterface, Fmgc {
  ...
  private _defaulttaxiFuelWeight = 0.5;
  ...
  public tryUpdateRouteAlternate() {
    if (this._DistanceToAlt < 20) {
      ...
      if (50 < airDistance && airDistance < 1200 && 100 < placeholderFl && placeholderFl < 290) {
  ...
  public tryUpdateRouteTrip(_dynamic = false) {
    ...
    if (20 <= airDistance && airDistance <= 3100 && 9400 <= altToUse && altToUse <= 410) {
      const deviation =
  ...
  public setPerfApprVApp(s: string): boolean {
    if (s === Keypad.clrValue) {
      ...
      if (isFinite(value) && value >= 90 && value <= 330) {
        this.vApp = value;
        return true;
      }
      this.setScratchpadMessage(NXSystemMessages.entryOutOfRange);
  ...
  public getFOB(): number | undefined {
    ...
    return useFqi
      ? (SimVar.GetSimVarValue('L:A32NX_TOTAL_FUEL_QUANTITY', 'number') * 0.4535934) / 1000
      : this.blockFuel;
  }
   ...
   */
  protected abstract setRequest(subsystem: 'ATSU' | 'ACARS' | 'ACMS' | 'CMS' | 'SAT' | 'FMGC'): void;
