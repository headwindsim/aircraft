...
// Struct to hold all relevant light levels for the A330X
struct LightingValues_A330X {
  // EFB
 ...
 * lighting values.
 */
class LightingPresets_A330X : public LightingPresets {
 private:
  // Lighting LVARs
  ...
  LightingValues_A330X currentLightValues{};
  // The lighting values that are loaded from the ini-file
  LightingValues_A330X loadedLightValues{};
  // The lighting values that are used to converge from the current values to the preset values
  LightingValues_A330X intermediateLightValues{};

 public:
  LightingPresets_A330X() = delete;

  /**
   * Creates a new LightingPresets_A330X instance and takes a reference to the MsfsHandler instance.
   * @param msfsHandler The MsfsHandler instance that is used to communicate with the simulator.
   */
  explicit LightingPresets_A330X(MsfsHandler& msfsHandler) : LightingPresets(msfsHandler) {}
 ...
 private:
  ...
  const LightingValues_A330X DEFAULT_50 = {
      50.0,  // efbBrightness
...
inline bool operator==(const LightingValues_A330X& p1, const LightingValues_A330X& p2) {
