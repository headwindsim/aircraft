...
#include "LightingPresets_A330X.h"
...
// The LightingPresets_A330X module uses the DataManager to get and set variables.
...
// LightingPresets_A330X will be dormant most of the time, this is saving a lot of
// unnecessary reads/writes.
///

bool LightingPresets_A330X::initialize_aircraft() {
  ...
  _isInitialized = true;
  LOG_INFO("LightingPresets_A330X initialized");
  return true;
}
...
void LightingPresets_A330X::readFromAircraft() {
...
void LightingPresets_A330X::applyToAircraft() {
...
void LightingPresets_A330X::loadFromIni(const mINI::INIStructure& ini, const std::string& iniSectionName) {
...
void LightingPresets_A330X::saveToIni(mINI::INIStructure& ini, const std::string& iniSectionName) const {
...
[[maybe_unused]] std::string LightingPresets_A330X::str() const {
...
void LightingPresets_A330X::setValidCabinLightValue(FLOAT64 level) {
  ...
  // cabin lights in the A330X need to be controlled by two vars
...
bool LightingPresets_A330X::calculateIntermediateValues(FLOAT64 stepSize) {
  // clang-format off
