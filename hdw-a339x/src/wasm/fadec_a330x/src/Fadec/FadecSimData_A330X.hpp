...
class FadecSimData_A330X {
 public:
  ...
  struct FuelTankData {
    FLOAT64 fuelSystemCenter;      // in Gallons
    FLOAT64 fuelSystemLeftOuter;   // in Gallons
    FLOAT64 fuelSystemRightOuter;  // in Gallons
    //FLOAT64 fuelSystemTrim;        // in Gallons // TODO: ADD TRIM SYSTEM
  };
  DataDefinitionVector fuelTankDataDef = {
      ...
      // {"FUELSYSTEM TANK QUANTITY", 6,  UNITS.Gallons}, // TODO: ADD TRIM SYSTEM
  ...
  DataDefinitionVariablePtr<CorrectedN1Data> engineCorrectedN1DataPtr[2];
...
// Corrected N3 Data in separate Data Definitions as they are updated separately
  ...
  struct SimVarsData {
    ...
    FLOAT64 engineAntiIce[2];        // 0 or 1
    FLOAT64 engineIgniter[2];        // 0 or 1
    FLOAT64 engineStarter[2];        // 0 or 1
  ...
  // SimVars
  AircraftVariablePtr engineCombustion[2];  // 0 or 1
  AircraftVariablePtr engineTime[2];        // in Seconds

  // LVars
  NamedVariablePtr airlinerToFlexTemp;  // Celsius
  NamedVariablePtr apuRpmPercent;       // Percent
  NamedVariablePtr engineEgt[2];        // Celsius
  NamedVariablePtr engineFF[2];         // kg/hour
  NamedVariablePtr engineFuelUsed[2];   // kg
  ...
  NamedVariablePtr engineN1[2];   // Percent
  NamedVariablePtr engineN2[2];   // Percent
  NamedVariablePtr engineN3[2];   // Percent
  ...
  // NamedVariablePtr fuelTrimPre;        // Pounds TODO
  ...
  void initLvars(DataManager* dm) {
    ...
    //fuelTrimPre       = dm->make_named_var("A32NX_FUEL_TRIM_PRE", UNITS.Number, AUTO_READ_WRITE); // TODO: ADD TRIM
