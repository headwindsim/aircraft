...
void AutopilotLawsModelClass::step()
{
  ...
  rtb_error_c = 0.017453292519943295 * rtb_GainTheta1;
  a = std::tan(b_R);
  distance_m = std::sin(rtb_error_c);
  rtb_error_c = std::cos(rtb_error_c);
  result_tmp[0] = 1.0;
  result_tmp[3] = distance_m * a;
  result_tmp[6] = rtb_error_c * a;
  result_tmp[1] = 0.0;
  result_tmp[4] = rtb_error_c;
  result_tmp[7] = -distance_m;
  result_tmp[2] = 0.0;
  Phi2 = std::cos(b_R);
  rtb_Sum_kq = 1.0 / Phi2;
  result_tmp[5] = rtb_Sum_kq * distance_m;
  result_tmp[8] = rtb_Sum_kq * rtb_error_c;
  rtb_dme = AutopilotLaws_P.Gain_Gain_de * AutopilotLaws_U.in.data.p_rad_s * AutopilotLaws_P.Gainpk_Gain;
  rtb_Saturation = AutopilotLaws_P.Gain_Gain_d * AutopilotLaws_U.in.data.q_rad_s * AutopilotLaws_P.Gainqk_Gain;
  a = AutopilotLaws_P.Gain_Gain_m * AutopilotLaws_U.in.data.r_rad_s;
  for (i = 0; i < 3; i++) {
    result[i] = (result_tmp[i + 3] * rtb_Saturation + result_tmp[i] * rtb_dme) + result_tmp[i + 6] * a;
  }
  ...
  result_tmp[1] = distance_m * rtb_dme;
  result_tmp[4] = rtb_error_c;
  result_tmp[7] = Phi2 * distance_m;
  result_tmp[2] = rtb_error_c * rtb_dme;
  result_tmp[5] = 0.0 - distance_m;
  result_tmp[8] = rtb_error_c * Phi2;
  for (i = 0; i < 3; i++) {
  ...
  b_L = mod_HV94nWDg((mod_HV94nWDg(mod_HV94nWDg(360.0) + 360.0) - (mod_HV94nWDg(mod_HV94nWDg
    (AutopilotLaws_U.in.data.nav_loc_magvar_deg) + 360.0) + 360.0)) + 360.0);
  b_R = mod_HV94nWDg(360.0 - b_L);
  if (std::abs(b_L) < std::abs(b_R)) {
    b_R = -b_L;
  }

  rtb_Sum_kq = std::cos(rtb_error_c);
  rtb_error_c = std::sin(rtb_error_c);
  L = mod_HV94nWDg(mod_HV94nWDg(mod_HV94nWDg(std::atan2(std::sin(R) * L, rtb_Sum_kq * std::sin(Phi2) - rtb_error_c * L *
    std::cos(R)) * 57.295779513082323 + 360.0)) + 360.0) + 360.0;
  Phi2 = mod_HV94nWDg((mod_HV94nWDg(mod_HV94nWDg(mod_HV94nWDg(mod_HV94nWDg(AutopilotLaws_U.in.data.nav_loc_deg - b_R) +
    360.0)) + 360.0) - L) + 360.0);
  b_R = mod_HV94nWDg(360.0 - Phi2);
  guard1 = false;
  if (std::sqrt(distance_m * distance_m + a * a) / 1852.0 < 30.0) {
    L = mod_HV94nWDg((mod_HV94nWDg(mod_HV94nWDg(AutopilotLaws_U.in.data.nav_loc_deg) + 360.0) - L) + 360.0);
    R = mod_HV94nWDg(360.0 - L);
  ...
  distance_m = std::sin((AutopilotLaws_U.in.data.nav_gs_position.lon - AutopilotLaws_U.in.data.aircraft_position.lon) *
                        0.017453292519943295 / 2.0);
  L = std::cos(Phi2);
  a = rtb_Sum_kq * L * distance_m * distance_m + a * a;
  distance_m = std::atan2(std::sqrt(a), std::sqrt(1.0 - a)) * 2.0 * 6.371E+6;
  ...
  rtb_error_c = std::atan2(std::sin(rtb_Saturation) * L, rtb_Sum_kq * std::sin(Phi2) - rtb_error_c * L * std::cos
    (rtb_Saturation)) * 57.295779513082323;
  if (rtb_error_c + 360.0 == 0.0) {
    rtb_Saturation = 0.0;
  } else {
    rtb_Saturation = std::fmod(rtb_error_c + 360.0, 360.0);
    if (rtb_Saturation == 0.0) {
      rtb_Saturation = 0.0;
    } else if (rtb_error_c + 360.0 < 0.0) {
  ...
  if (distance_m / 1852.0 < 30.0) {
    if (AutopilotLaws_U.in.data.nav_loc_deg == 0.0) {
      ...
      rtb_error_c = std::fmod(AutopilotLaws_U.in.data.nav_loc_deg, 360.0);
      if (rtb_error_c == 0.0) {
        rtb_error_c = 0.0;
    ...
    if (rtb_error_c + 360.0 == 0.0) {
      ...
      rtb_Sum_kq = std::fmod(rtb_error_c + 360.0, 360.0);
    }

    if (Phi2 + 360.0 == 0.0) {
      rtb_error_c = 0.0;
    } else {
      rtb_error_c = std::fmod(Phi2 + 360.0, 360.0);
    }

    rtb_error_c = (rtb_Sum_kq - (rtb_error_c + 360.0)) + 360.0;
    if (rtb_error_c == 0.0) {
      L = 0.0;
    } else {
      L = std::fmod(rtb_error_c, 360.0);
      if (L == 0.0) {
        L = 0.0;
  ...
  AutopilotLaws_DWork.DelayInput1_DSTATE = rt_modd(AutopilotLaws_DWork.DelayInput1_DSTATE,
    AutopilotLaws_P.Constant3_Value_b);
  rtb_error_c = AutopilotLaws_U.in.data.nav_loc_deg - AutopilotLaws_U.in.data.nav_loc_magvar_deg;
  R = rt_modd(rt_modd(rtb_error_c, AutopilotLaws_P.Constant3_Value_n) + AutopilotLaws_P.Constant3_Value_n,
  ...
  if (b_R < a) {
    rtb_Sum_kq = AutopilotLaws_P.Gain1_Gain * b_R;
  } else {
    rtb_Sum_kq = AutopilotLaws_P.Gain_Gain * a;
  }

  b_R = std::abs(rtb_Sum_kq);
  ...
  AutopilotLaws_MATLABFunction(AutopilotLaws_P.tau_Value, AutopilotLaws_P.zeta_Value, &a, &b_L);
  if (rtb_dme > AutopilotLaws_P.Saturation_UpperSat_b) {
    rtb_Sum_kq = AutopilotLaws_P.Saturation_UpperSat_b;
  } else if (rtb_dme < AutopilotLaws_P.Saturation_LowerSat_n) {
    rtb_Sum_kq = AutopilotLaws_P.Saturation_LowerSat_n;
  } else {
    rtb_Sum_kq = rtb_dme;
  }

  b_R = std::sin(AutopilotLaws_P.Gain1_Gain_f * AutopilotLaws_U.in.data.nav_loc_error_deg) * rtb_Sum_kq *
  ...
  AutopilotLaws_RateLimiter_n(rtb_Saturation == AutopilotLaws_P.CompareToConstant1_const,
    ...
    AutopilotLaws_P.RateLimiterVariableTs_InitialCondition, &rtb_Y_i0, &AutopilotLaws_DWork.sf_RateLimiter_n);
  ...
  AutopilotLaws_DWork.DelayInput1_DSTATE = rtb_error_c;
  AutopilotLaws_DWork.DelayInput1_DSTATE = rt_modd(AutopilotLaws_DWork.DelayInput1_DSTATE,
    AutopilotLaws_P.Constant3_Value_if);
  AutopilotLaws_DWork.DelayInput1_DSTATE += AutopilotLaws_P.Constant3_Value_if;
  distance_m = rt_modd(AutopilotLaws_DWork.DelayInput1_DSTATE, AutopilotLaws_P.Constant3_Value_if);
  ...
  rtb_error_c = rt_modd(AutopilotLaws_DWork.DelayInput1_DSTATE, AutopilotLaws_P.Constant3_Value_cd);
  AutopilotLaws_DWork.DelayInput1_DSTATE = AutopilotLaws_P.Constant3_Value_cd - rtb_error_c;
  AutopilotLaws_DWork.DelayInput1_DSTATE = rt_modd(AutopilotLaws_DWork.DelayInput1_DSTATE,
    AutopilotLaws_P.Constant3_Value_cd);
  ...
  if (a < 1.0) {
    ...
    rtb_valid_m = AutopilotLaws_DWork.Delay_DSTATE_l[100U - static_cast<uint32_T>(i)];
  }

  AutopilotLaws_Chart(rtb_error_c, AutopilotLaws_P.Gain_Gain_cy * AutopilotLaws_DWork.DelayInput1_DSTATE, rtb_valid !=
                      rtb_valid_m, &a, &AutopilotLaws_DWork.sf_Chart);
  ...
  rtb_error_c = a * AutopilotLaws_DWork.DelayInput1_DSTATE;
  AutopilotLaws_DWork.DelayInput1_DSTATE = AutopilotLaws_P.Gain_Gain_o * result[2];
  L = AutopilotLaws_P.Gain1_Gain_o * rtb_error_c + AutopilotLaws_DWork.DelayInput1_DSTATE;
  AutopilotLaws_MATLABFunction_m(AutopilotLaws_U.in.input.Psi_c_deg, a, L, &rtb_error_c, &rtb_lo_f,
  ...
  AutopilotLaws_DWork.DelayInput1_DSTATE = rt_modd(AutopilotLaws_DWork.DelayInput1_DSTATE,
    AutopilotLaws_P.Constant3_Value_k);
  rtb_valid_m = ((rtb_Saturation == AutopilotLaws_P.CompareToConstant4_const) ==
  ...
  if (a < 1.0) {
    ...
    rtb_Delay_j = AutopilotLaws_DWork.Delay_DSTATE_h5[100U - static_cast<uint32_T>(i)];
  }

  AutopilotLaws_Chart(R, AutopilotLaws_P.Gain_Gain_p * AutopilotLaws_DWork.DelayInput1_DSTATE, rtb_valid_m !=
                      rtb_Delay_j, &a, &AutopilotLaws_DWork.sf_Chart_ba);
  AutopilotLaws_DWork.DelayInput1_DSTATE = look1_binlxpw(AutopilotLaws_U.in.data.V_tas_kn,
  ...
  AutopilotLaws_MATLABFunction_m(AutopilotLaws_U.in.input.Psi_c_deg, a, rtb_Sum_i, &rtb_Y_j, &b_L,
  ...
  switch (static_cast<int32_T>(rtb_Saturation)) {
   ...
   case 3:
    rtb_Add3_lz = AutopilotLaws_P.Gain_Gain_c * AutopilotLaws_U.in.data.flight_guidance_xtk_nmi * R /
      AutopilotLaws_U.in.data.V_gnd_kn;
    if (rtb_Add3_lz > AutopilotLaws_P.Saturation_UpperSat) {
      rtb_Add3_lz = AutopilotLaws_P.Saturation_UpperSat;
    } else if (rtb_Add3_lz < AutopilotLaws_P.Saturation_LowerSat) {
      rtb_Add3_lz = AutopilotLaws_P.Saturation_LowerSat;
    }

    L = a - (AutopilotLaws_P.Gain2_Gain * AutopilotLaws_U.in.data.flight_guidance_tae_deg + rtb_Add3_lz) *
   ...
   case 5:
    rtb_error_c = rt_modd((AutopilotLaws_U.in.data.Psi_magnetic_deg - (AutopilotLaws_U.in.data.Psi_true_deg +
      AutopilotLaws_P.Constant3_Value)) + AutopilotLaws_P.Constant3_Value, AutopilotLaws_P.Constant3_Value);
    a = rt_modd(AutopilotLaws_P.Constant3_Value - rtb_error_c, AutopilotLaws_P.Constant3_Value);
    if (rtb_error_c < a) {
      rtb_Sum_kq = AutopilotLaws_P.Gain1_Gain_l * rtb_error_c;
    } else {
      rtb_Sum_kq = AutopilotLaws_P.Gain_Gain_g * a;
    }

    rtb_error_c = rt_modd((rt_modd(rt_modd(AutopilotLaws_U.in.data.Psi_magnetic_track_deg + rtb_Sum_kq,
      AutopilotLaws_P.Constant3_Value_d) + AutopilotLaws_P.Constant3_Value_d, AutopilotLaws_P.Constant3_Value_d) -
      (distance_m + AutopilotLaws_P.Constant3_Value_c)) + AutopilotLaws_P.Constant3_Value_c,
                          AutopilotLaws_P.Constant3_Value_c);
    a = rt_modd(AutopilotLaws_P.Constant3_Value_c - rtb_error_c, AutopilotLaws_P.Constant3_Value_c);
    if (rtb_Y_i0 > AutopilotLaws_P.Saturation_UpperSat_a) {
      rtb_Y_i0 = AutopilotLaws_P.Saturation_UpperSat_a;
    } else if (rtb_Y_i0 < AutopilotLaws_P.Saturation_LowerSat_a) {
      rtb_Y_i0 = AutopilotLaws_P.Saturation_LowerSat_a;
    }

    if (rtb_error_c < a) {
      rtb_Sum_kq = AutopilotLaws_P.Gain1_Gain_g * rtb_error_c;
    } else {
      rtb_Sum_kq = AutopilotLaws_P.Gain_Gain_f * a;
    }

    if (rtb_Compare_jy) {
      rtb_error_c = AutopilotLaws_P.k_beta_Phi_Gain * AutopilotLaws_U.in.data.beta_deg;
    } else {
      rtb_error_c = AutopilotLaws_P.Constant1_Value_fk;
    }

    rtb_Add3_lz = (rtb_Y_i * look1_binlxpw(AutopilotLaws_U.in.data.V_tas_kn,
      ...
      AutopilotLaws_P.ScheduledGain_BreakpointsForDimension1, AutopilotLaws_P.ScheduledGain_Table, 5U) + std::sin
                   (AutopilotLaws_P.Gain1_Gain_b * rtb_Sum_kq) * AutopilotLaws_U.in.data.V_gnd_kn *
                   AutopilotLaws_P.Gain2_Gain_g) + rtb_error_c;
    if (rtb_Add3_lz > AutopilotLaws_P.Saturation1_UpperSat) {
      rtb_Add3_lz = AutopilotLaws_P.Saturation1_UpperSat;
    } else if (rtb_Add3_lz < AutopilotLaws_P.Saturation1_LowerSat) {
      rtb_Add3_lz = AutopilotLaws_P.Saturation1_LowerSat;
    }

    L = (AutopilotLaws_P.Constant_Value - rtb_Y_i0) * Phi2 + rtb_Add3_lz * rtb_Y_i0;
    break;
  ...
  rtb_error_c = std::abs(AutopilotLaws_U.in.data.V_tas_kn);
  if (rtb_error_c > 600.0) {
    rtb_error_c = 19.0;
  } else {
    int32_T low_i;
    int32_T low_ip1;
    ...
    while (i > low_ip1) {
      int32_T mid_i;
      mid_i = (low_i + i) >> 1;
      if (rtb_error_c >= (static_cast<real_T>(mid_i) - 1.0) * 150.0) {
        low_i = mid_i;
        low_ip1 = mid_i + 1;
      } else {
        i = mid_i;
    ...
    rtb_error_c = (rtb_error_c - (static_cast<real_T>(low_i) - 1.0) * 150.0) / static_cast<real_T>(150 * low_i - (low_i
      - 1) * 150);
    if (rtb_error_c == 0.0) {
      rtb_error_c = b[low_i - 1];
    } else if (rtb_error_c == 1.0) {
      rtb_error_c = b[low_i];
    } else if (b[low_i - 1] == b[low_i]) {
      rtb_error_c = b[low_i - 1];
    } else {
      rtb_error_c = (1.0 - rtb_error_c) * static_cast<real_T>(b[low_i - 1]) + rtb_error_c * static_cast<real_T>(b[low_i]);
    }
  }
  ...
  if ((AutopilotLaws_U.in.input.lateral_mode != 30.0) && (AutopilotLaws_U.in.input.lateral_mode != 31.0) &&
    ...
    rtb_error_c = std::fmin(25.0, rtb_error_c);
  } else if (AutopilotLaws_U.in.data.H_radio_ft < 700.0) {
    rtb_error_c = 10.0;
  }

  a = std::abs(AutopilotLaws_U.in.data.flight_guidance_phi_limit_deg);
  if (!AutopilotLaws_DWork.pY_not_empty_j) {
    AutopilotLaws_DWork.pY_e = 25.0;
    AutopilotLaws_DWork.pY_not_empty_j = true;
  }
  ...
  AutopilotLaws_DWork.pY_e += std::fmax(std::fmin(rtb_error_c - AutopilotLaws_DWork.pY_e, 5.0 *
    AutopilotLaws_U.in.time.dt), -5.0 * AutopilotLaws_U.in.time.dt);
  if (L > AutopilotLaws_DWork.pY_e) {
    AutopilotLaws_DWork.DelayInput1_DSTATE = AutopilotLaws_DWork.pY_e;
  } else {
    AutopilotLaws_DWork.DelayInput1_DSTATE = AutopilotLaws_P.Gain1_Gain_lt * AutopilotLaws_DWork.pY_e;
  ...
  rtb_Delay_j = (rtb_Saturation == AutopilotLaws_P.CompareToConstant_const_k);
  if (!rtb_Delay_j) {
  ...
  AutopilotLaws_storevalue(rtb_Delay_j, rt_modd(rt_modd(AutopilotLaws_U.in.data.nav_loc_deg -
    AutopilotLaws_U.in.data.nav_loc_magvar_deg, AutopilotLaws_P.Constant3_Value_dk) + AutopilotLaws_P.Constant3_Value_dk,
    AutopilotLaws_P.Constant3_Value_dk), &rtb_Y_j, &AutopilotLaws_DWork.sf_storevalue);
  rtb_error_c = rt_modd((AutopilotLaws_U.in.data.Psi_true_deg - (rt_modd(rt_modd
    (AutopilotLaws_U.in.data.nav_loc_error_deg + rtb_Y_j, AutopilotLaws_P.Constant3_Value_o) +
  ...
  rtb_dme = rt_modd(AutopilotLaws_P.Constant3_Value_n1 - rtb_error_c, AutopilotLaws_P.Constant3_Value_n1);
  if (rtb_error_c < rtb_dme) {
    rtb_Sum_kq = AutopilotLaws_P.Gain1_Gain_j * rtb_error_c;
  } else {
    rtb_Sum_kq = AutopilotLaws_P.Gain_Gain_i * rtb_dme;
  }
  ...
  rtb_dme = rt_modd((rt_modd(rt_modd(((R * look1_binlxpw(AutopilotLaws_U.in.data.V_gnd_kn,
    ...
    AutopilotLaws_DWork.Delay_DSTATE_h) + AutopilotLaws_P.Gain1_Gain_fq * rtb_Sum_kq) +
  ...
  AutopilotLaws_Chart_h(rtb_dme, AutopilotLaws_P.Gain_Gain_oc * rt_modd(AutopilotLaws_P.Constant3_Value_nr - rtb_dme,
    AutopilotLaws_P.Constant3_Value_nr), AutopilotLaws_P.Constant1_Value_e, &rtb_error_c,
                        &AutopilotLaws_DWork.sf_Chart_b);
  AutopilotLaws_RateLimiter_n(rtb_Delay_j, AutopilotLaws_P.RateLimiterVariableTs_up_n,
  ...
  rtb_error_c = (AutopilotLaws_P.Gain_Gain_b * result[2] * rtb_Sum_kq + (AutopilotLaws_P.Constant_Value_a - rtb_Sum_kq) *
                 (AutopilotLaws_P.Gain4_Gain_o * AutopilotLaws_U.in.data.beta_deg)) + AutopilotLaws_P.Gain5_Gain_o *
    rtb_error_c;
  if (rtb_fpmtoms > AutopilotLaws_P.Switch_Threshold_n) {
    switch (static_cast<int32_T>(rtb_Saturation)) {
     ...
     default:
      R = AutopilotLaws_P.Gain3_Gain * rtb_error_c;
      break;
    }
  } else {
  ...
  rtb_dme = rt_modd((distance_m - (AutopilotLaws_U.in.data.Psi_true_deg + AutopilotLaws_P.Constant3_Value_m)) +
  ...
  AutopilotLaws_Chart_h(rtb_dme, AutopilotLaws_P.Gain_Gain_fn * rt_modd(AutopilotLaws_P.Constant3_Value_m - rtb_dme,
    AutopilotLaws_P.Constant3_Value_m), AutopilotLaws_P.Constant2_Value_l, &a, &AutopilotLaws_DWork.sf_Chart_h);
  if (rtb_Compare_jy) {
    rtb_Sum_kq = AutopilotLaws_P.Gain_Gain_ae * a + AutopilotLaws_P.Gain1_Gain_k * AutopilotLaws_U.in.data.beta_deg;
  } else {
    rtb_Sum_kq = AutopilotLaws_P.Constant1_Value_fk;
  }

  AutopilotLaws_LagFilter(rtb_Sum_kq, AutopilotLaws_P.LagFilter1_C1, AutopilotLaws_U.in.time.dt, &rtb_Y_j,
  ...
  switch (static_cast<int32_T>(rtb_Saturation)) {
   ...
   default:
    rtb_Saturation = AutopilotLaws_P.Gain7_Gain * rtb_error_c;
    break;
  }

  AutopilotLaws_LagFilter(rtb_Saturation, AutopilotLaws_P.LagFilter_C1_k, AutopilotLaws_U.in.time.dt, &rtb_error_c,
  ...
  AutopilotLaws_DWork.pY += std::fmax(std::fmin(rtb_Sum_kq - AutopilotLaws_DWork.pY, 2.5 * AutopilotLaws_U.in.time.dt),
  ...
  AutopilotLaws_LagFilter(AutopilotLaws_DWork.Delay_DSTATE_hc, AutopilotLaws_P.LagFilter_C1_l,
    AutopilotLaws_U.in.time.dt, &rtb_Y_i0, &AutopilotLaws_DWork.sf_LagFilter_o);
  ...
  rtb_dme = rtb_Y_i0 * AutopilotLaws_DWork.DelayInput1_DSTATE;
  ...
  rtb_Add3_lz = AutopilotLaws_P.Gain_Gain_m3 * R;
  if (rtb_Add3_lz > AutopilotLaws_P.Saturation_UpperSat_c) {
    AutopilotLaws_Y.out.output.Nosewheel_c = AutopilotLaws_P.Saturation_UpperSat_c;
  } else if (rtb_Add3_lz < AutopilotLaws_P.Saturation_LowerSat_d) {
    AutopilotLaws_Y.out.output.Nosewheel_c = AutopilotLaws_P.Saturation_LowerSat_d;
  } else {
    AutopilotLaws_Y.out.output.Nosewheel_c = rtb_Add3_lz;
  }

  AutopilotLaws_Y.out.output.flight_director.Beta_c_deg = rtb_error_c;
  ...
  AutopilotLaws_WashoutFilter(rtb_GainTheta, AutopilotLaws_P.WashoutFilter_C1, AutopilotLaws_U.in.time.dt, &rtb_error_c,
  ...
  rtb_Add3_lz = a / AutopilotLaws_DWork.DelayInput1_DSTATE;
  if (rtb_Add3_lz > 1.0) {
    rtb_Add3_lz = 1.0;
  } else if (rtb_Add3_lz < -1.0) {
    rtb_Add3_lz = -1.0;
  }

  Phi2 = AutopilotLaws_P.Gain_Gain_k * std::asin(rtb_Add3_lz);
  rtb_Compare_jy = (rtb_dme == AutopilotLaws_P.CompareToConstant1_const_c);
  if (!AutopilotLaws_DWork.wasActive_not_empty_m) {
    AutopilotLaws_DWork.wasActive_o = rtb_Compare_jy;
  ...
  if (distance_m < 0.0) {
    ...
    rtb_Sum_kq = (distance_m > 0.0);
  }

  a = rtb_Sum_kq * AutopilotLaws_DWork.dH_offset + distance_m;
  if ((!AutopilotLaws_DWork.wasActive_o) && rtb_Compare_jy) {
    ...
    a += rtb_Sum_kq * AutopilotLaws_DWork.dH_offset;
    AutopilotLaws_DWork.k = AutopilotLaws_U.in.data.H_dot_ft_min / a;
  ...
  if (std::abs(a) > AutopilotLaws_DWork.maxH_dot) {
    ...
    a = rtb_Sum_kq * AutopilotLaws_DWork.maxH_dot;
  }
  ...
  a = AutopilotLaws_P.Gain_Gain_es * std::asin(rtb_Add3_lz);
  AutopilotLaws_DWork.DelayInput1_DSTATE = AutopilotLaws_P.fpmtoms_Gain * AutopilotLaws_U.in.data.H_dot_ft_min;
  rtb_Add3_lz = AutopilotLaws_P.kntoms_Gain_m * AutopilotLaws_U.in.data.V_gnd_kn;
  if (rtb_Add3_lz > AutopilotLaws_P.Saturation_UpperSat_j) {
    rtb_Add3_lz = AutopilotLaws_P.Saturation_UpperSat_j;
  } else if (rtb_Add3_lz < AutopilotLaws_P.Saturation_LowerSat_i) {
    rtb_Add3_lz = AutopilotLaws_P.Saturation_LowerSat_i;
  }

  AutopilotLaws_DWork.DelayInput1_DSTATE = std::atan(AutopilotLaws_DWork.DelayInput1_DSTATE / rtb_Add3_lz) *
  ...
  rtb_Add3_lz = AutopilotLaws_P.kntoms_Gain_b * AutopilotLaws_U.in.data.V_gnd_kn;
  if (rtb_Add3_lz > AutopilotLaws_P.Saturation_UpperSat_ei) {
    rtb_Add3_lz = AutopilotLaws_P.Saturation_UpperSat_ei;
  } else if (rtb_Add3_lz < AutopilotLaws_P.Saturation_LowerSat_dz) {
    rtb_Add3_lz = AutopilotLaws_P.Saturation_LowerSat_dz;
  }
  ...
  AutopilotLaws_LeadLagFilter(rtb_Y_j - AutopilotLaws_P.g_Gain * (AutopilotLaws_P.Gain1_Gain_lp *
    ...
    rtb_Add3_lz))) * (AutopilotLaws_P.Constant_Value_dy - std::cos(R)) + std::sin(R) * std::sin
    ...
    AutopilotLaws_P.HighPassFilter_C3, AutopilotLaws_P.HighPassFilter_C4, AutopilotLaws_U.in.time.dt, &rtb_Y_i0,
  ...
  R = (rtb_Y_i0 + rtb_Y_j) * AutopilotLaws_P.ug_Gain;
  ...
  rtb_lo_f = (AutopilotLaws_P.Gain_Gain_b0 * L - AutopilotLaws_DWork.DelayInput1_DSTATE) + R;
  AutopilotLaws_DWork.DelayInput1_DSTATE = AutopilotLaws_P.fpmtoms_Gain_a * AutopilotLaws_U.in.data.H_dot_ft_min;
  rtb_Add3_lz = AutopilotLaws_P.kntoms_Gain_p * AutopilotLaws_U.in.data.V_gnd_kn;
  if (rtb_Add3_lz > AutopilotLaws_P.Saturation_UpperSat_h) {
    rtb_Add3_lz = AutopilotLaws_P.Saturation_UpperSat_h;
  } else if (rtb_Add3_lz < AutopilotLaws_P.Saturation_LowerSat_e) {
    rtb_Add3_lz = AutopilotLaws_P.Saturation_LowerSat_e;
  }
  ...
  rtb_Add3_lz = AutopilotLaws_P.kntoms_Gain_l * AutopilotLaws_U.in.data.V_gnd_kn;
  if (rtb_Add3_lz > AutopilotLaws_P.Saturation_UpperSat_i) {
    rtb_Add3_lz = AutopilotLaws_P.Saturation_UpperSat_i;
  } else if (rtb_Add3_lz < AutopilotLaws_P.Saturation_LowerSat_h) {
    rtb_Add3_lz = AutopilotLaws_P.Saturation_LowerSat_h;
  }
  ...
  AutopilotLaws_LeadLagFilter(rtb_Y_j - AutopilotLaws_P.g_Gain_h * (AutopilotLaws_P.Gain1_Gain_dv *
    ...
    rtb_Add3_lz))) * (AutopilotLaws_P.Constant_Value_c - std::cos(R)) + std::sin(R) * std::sin
    ...
    AutopilotLaws_U.in.time.dt, &rtb_Y_i0, &AutopilotLaws_DWork.sf_LeadLagFilter_h);
  ...
  R = (rtb_Y_i0 + rtb_Y_j) * AutopilotLaws_P.ug_Gain_a;
  ...
  rtb_Gain1_na = (AutopilotLaws_P.Gain1_Gain_mz * R + rtb_Sum_kq) * AutopilotLaws_P.Gain_Gain_ie;
  ...
  rtb_Sum_i = rtb_Y_j * AutopilotLaws_P.Gain1_Gain_f1;
  if (rtb_Sum_i <= R) {
  ...
  AutopilotLaws_SpeedProtectionSignalSelection(&AutopilotLaws_Y.out, a, AutopilotLaws_P.VS_Gain * a, rtb_lo_f,
    AutopilotLaws_P.Gain_Gain_m0 * rtb_lo_f, R, AutopilotLaws_P.Gain_Gain_lr * R, AutopilotLaws_P.Constant_Value_ig,
    &b_L, &L);
  AutopilotLaws_DWork.DelayInput1_DSTATE = AutopilotLaws_P.fpmtoms_Gain_p * AutopilotLaws_U.in.data.H_dot_ft_min;
  rtb_Add3_lz = AutopilotLaws_P.kntoms_Gain_f * AutopilotLaws_U.in.data.V_gnd_kn;
  if (rtb_Add3_lz > AutopilotLaws_P.Saturation_UpperSat_eik) {
    rtb_Add3_lz = AutopilotLaws_P.Saturation_UpperSat_eik;
  } else if (rtb_Add3_lz < AutopilotLaws_P.Saturation_LowerSat_ad) {
    rtb_Add3_lz = AutopilotLaws_P.Saturation_LowerSat_ad;
  }
  ...
  rtb_Add3_lz = AutopilotLaws_P.kntoms_Gain_a * AutopilotLaws_U.in.data.V_gnd_kn;
  if (rtb_Add3_lz > AutopilotLaws_P.Saturation_UpperSat_f) {
    rtb_Add3_lz = AutopilotLaws_P.Saturation_UpperSat_f;
  } else if (rtb_Add3_lz < AutopilotLaws_P.Saturation_LowerSat_c) {
    rtb_Add3_lz = AutopilotLaws_P.Saturation_LowerSat_c;
  }
  ...
  AutopilotLaws_LeadLagFilter(rtb_Gain1_na - AutopilotLaws_P.g_Gain_j * (AutopilotLaws_P.Gain1_Gain_ca *
    ...
    rtb_Add3_lz))) * (AutopilotLaws_P.Constant_Value_l - rtb_lo_f) + rtb_Sum_i * std::sin(AutopilotLaws_P.Gain1_Gain_id *
  ...
  rtb_lo_f = (AutopilotLaws_P.Gain1_Gain_hu * a + R) * AutopilotLaws_P.Gain_Gain_bn;
  ...
  rtb_Compare_jy = ((distance_m > AutopilotLaws_P.CompareToConstant6_const) && (rtb_lo_f <
  ...
  if (rtb_Compare_jy) {
    ...
    if (distance_m > AutopilotLaws_P.CompareToConstant_const_l) {
      ...
      a = AutopilotLaws_P.Gain5_Gain_k * rtb_lo_f;
    }

    if (rtb_Gain1_na <= a) {
      if (distance_m > AutopilotLaws_P.CompareToConstant4_const_o) {
        a = std::fmax(AutopilotLaws_P.Constant2_Value, AutopilotLaws_P.Gain1_Gain_kg * rtb_lo_f);
      } else {
        a = AutopilotLaws_P.Gain6_Gain_a * rtb_lo_f;
      }
  ...
  rtb_Sum_i = (AutopilotLaws_P.Gain_Gain_d4y * R - AutopilotLaws_DWork.DelayInput1_DSTATE) + a;
  rtb_Add3_lz = AutopilotLaws_P.kntoms_Gain_n * AutopilotLaws_U.in.data.V_tas_kn;
  if (distance_m < 0.0) {
    rtb_Sum_kq = -1.0;
  } else {
    rtb_Sum_kq = (distance_m > 0.0);
  ...
  if (rtb_Add3_lz > AutopilotLaws_P.Saturation_UpperSat_ju) {
    rtb_Add3_lz = AutopilotLaws_P.Saturation_UpperSat_ju;
  } else if (rtb_Add3_lz < AutopilotLaws_P.Saturation_LowerSat_gw) {
    rtb_Add3_lz = AutopilotLaws_P.Saturation_LowerSat_gw;
  }

  rtb_Add3_lz = (rtb_Sum_kq * AutopilotLaws_P.Constant3_Value_ix - AutopilotLaws_U.in.data.H_dot_ft_min) *
    AutopilotLaws_P.ftmintoms_Gain_d / rtb_Add3_lz;
  if (rtb_Add3_lz > 1.0) {
    rtb_Add3_lz = 1.0;
  } else if (rtb_Add3_lz < -1.0) {
    rtb_Add3_lz = -1.0;
  ...
  rtb_Gain_ml = AutopilotLaws_P.Gain_Gain_nz * std::asin(rtb_Add3_lz);
  rtb_Add3_lz = AutopilotLaws_P.kntoms_Gain_au * AutopilotLaws_U.in.data.V_tas_kn;
  if (rtb_Add3_lz > AutopilotLaws_P.Saturation_UpperSat_l) {
    rtb_Add3_lz = AutopilotLaws_P.Saturation_UpperSat_l;
  } else if (rtb_Add3_lz < AutopilotLaws_P.Saturation_LowerSat_hm) {
    rtb_Add3_lz = AutopilotLaws_P.Saturation_LowerSat_hm;
  }

  rtb_Add3_lz = (AutopilotLaws_U.in.input.H_dot_c_fpm - AutopilotLaws_U.in.data.H_dot_ft_min) *
    AutopilotLaws_P.ftmintoms_Gain_l / rtb_Add3_lz;
  if (rtb_Add3_lz > 1.0) {
    rtb_Add3_lz = 1.0;
  } else if (rtb_Add3_lz < -1.0) {
    rtb_Add3_lz = -1.0;
  ...
  rtb_Y_i = AutopilotLaws_P.Gain_Gain_ey * std::asin(rtb_Add3_lz);
  ...
  rtb_lo_f = 9.81 / (AutopilotLaws_U.in.data.V_tas_kn * 0.51444444444444448);
  rtb_Y_i0 = rtb_lo_f * rtb_Sum_kq * 57.295779513082323;
  AutopilotLaws_DWork.prevVerticalLaw = AutopilotLaws_U.in.input.vertical_law;
  AutopilotLaws_DWork.prevTarget = AutopilotLaws_U.in.input.H_dot_c_fpm;
  ...
  rtb_Add3_lz = AutopilotLaws_P.kntoms_Gain_d * AutopilotLaws_U.in.data.V_gnd_kn;
  if (rtb_Add3_lz > AutopilotLaws_P.Saturation_UpperSat_hb) {
    rtb_Add3_lz = AutopilotLaws_P.Saturation_UpperSat_hb;
  } else if (rtb_Add3_lz < AutopilotLaws_P.Saturation_LowerSat_k) {
    rtb_Add3_lz = AutopilotLaws_P.Saturation_LowerSat_k;
  }

  R = AutopilotLaws_DWork.DelayInput1_DSTATE - std::atan(AutopilotLaws_P.fpmtoms_Gain_e *
    AutopilotLaws_U.in.data.H_dot_ft_min / rtb_Add3_lz) * AutopilotLaws_P.Gain_Gain_in * AutopilotLaws_P.Gain1_Gain_ps;
  ...
  rtb_Sum_kq = AutopilotLaws_P.Gain1_Gain_hg * AutopilotLaws_U.in.data.Psi_magnetic_track_deg -
  ...
  AutopilotLaws_LeadLagFilter(rtb_Gain1_na - AutopilotLaws_P.g_Gain_m * (AutopilotLaws_P.Gain1_Gain_kdq *
    ...
    (rtb_Sum_kq)))), AutopilotLaws_P.HighPassFilter_C1_g, AutopilotLaws_P.HighPassFilter_C2_l,
  ...
  rtb_Sum_kq = (AutopilotLaws_P.Gain_Gain_j * rtb_Cos_i - a) + AutopilotLaws_DWork.DelayInput1_DSTATE;
  ...
  rtb_Add3_lz = AutopilotLaws_P.kntoms_Gain_l5 * AutopilotLaws_U.in.data.V_gnd_kn;
  if (rtb_Add3_lz > AutopilotLaws_P.Saturation_UpperSat_b3) {
    rtb_Add3_lz = AutopilotLaws_P.Saturation_UpperSat_b3;
  } else if (rtb_Add3_lz < AutopilotLaws_P.Saturation_LowerSat_es) {
    rtb_Add3_lz = AutopilotLaws_P.Saturation_LowerSat_es;
  }

  R = AutopilotLaws_DWork.DelayInput1_DSTATE - std::atan(AutopilotLaws_P.fpmtoms_Gain_j *
    AutopilotLaws_U.in.data.H_dot_ft_min / rtb_Add3_lz) * AutopilotLaws_P.Gain_Gain_e5 * AutopilotLaws_P.Gain1_Gain_ja;
  ...
  rtb_Add3_np = AutopilotLaws_P.Gain1_Gain_ero * AutopilotLaws_U.in.data.Psi_magnetic_track_deg -
    AutopilotLaws_DWork.DelayInput1_DSTATE;
  ...
  AutopilotLaws_LeadLagFilter(rtb_Gain1_na - AutopilotLaws_P.g_Gain_g * (AutopilotLaws_P.Gain1_Gain_hv *
    ...
    (rtb_Add3_np)))), AutopilotLaws_P.HighPassFilter_C1_n, AutopilotLaws_P.HighPassFilter_C2_m,
  ...
  AutopilotLaws_SpeedProtectionSignalSelection(&AutopilotLaws_Y.out, rtb_Y_i, std::fmax(-rtb_Y_i0, std::fmin(rtb_Y_i0,
    AutopilotLaws_P.VS_Gain_h * rtb_Y_i)), rtb_Sum_kq, AutopilotLaws_P.Gain_Gain_h4 * rtb_Sum_kq, a,
  ...
  rtb_Add3_np = AutopilotLaws_U.in.input.FPA_c_deg - std::atan(AutopilotLaws_P.fpmtoms_Gain_ps *
  ...
  rtb_Add3_lz = AutopilotLaws_P.kntoms_Gain_k * AutopilotLaws_U.in.data.V_gnd_kn;
  if (rtb_Add3_lz > AutopilotLaws_P.Saturation_UpperSat_pj) {
    rtb_Add3_lz = AutopilotLaws_P.Saturation_UpperSat_pj;
  } else if (rtb_Add3_lz < AutopilotLaws_P.Saturation_LowerSat_py) {
    rtb_Add3_lz = AutopilotLaws_P.Saturation_LowerSat_py;
  }

  R = AutopilotLaws_DWork.DelayInput1_DSTATE - std::atan(AutopilotLaws_P.fpmtoms_Gain_f *
    AutopilotLaws_U.in.data.H_dot_ft_min / rtb_Add3_lz) * AutopilotLaws_P.Gain_Gain_bf * AutopilotLaws_P.Gain1_Gain_jv;
  AutopilotLaws_DWork.DelayInput1_DSTATE = AutopilotLaws_P.Gain1_Gain_gfa * rtb_GainTheta1;
  rtb_Sum_kq = std::cos(AutopilotLaws_DWork.DelayInput1_DSTATE);
  ...
  AutopilotLaws_LeadLagFilter(rtb_Y_i - AutopilotLaws_P.g_Gain_l * (AutopilotLaws_P.Gain1_Gain_n4 *
    (AutopilotLaws_P.Gain_Gain_bc * (R * (AutopilotLaws_P.Constant_Value_lf - rtb_Sum_kq) + rtb_Cos1_j * std::sin
  ...
  AutopilotLaws_DWork.DelayInput1_DSTATE = (rtb_Gain1_na + rtb_Y_i) * AutopilotLaws_P.ug_Gain_n;
  R = AutopilotLaws_P.Gain1_Gain_b1 * a;
  rtb_Gain1_na = AutopilotLaws_DWork.DelayInput1_DSTATE + R;
  rtb_Sum_kq = AutopilotLaws_P.Constant3_Value_nk - AutopilotLaws_P.Constant4_Value_o;
  R = (AutopilotLaws_P.Gain1_Gain_on * AutopilotLaws_DWork.DelayInput1_DSTATE + R) * AutopilotLaws_P.Gain_Gain_hy;
  if (rtb_Sum_kq > AutopilotLaws_P.Switch_Threshold_d) {
  ...
  if (rtb_Y_i <= AutopilotLaws_DWork.DelayInput1_DSTATE) {
    if (rtb_Sum_kq > AutopilotLaws_P.Switch1_Threshold_d) {
  ...
  AutopilotLaws_LeadLagFilter(rtb_Y_i - AutopilotLaws_P.g_Gain_hq * (AutopilotLaws_P.Gain1_Gain_mx *
    (AutopilotLaws_P.Gain_Gain_d3 * (R * (AutopilotLaws_P.Constant_Value_fo - rtb_Sum_kq) + rtb_Add3_j4 * std::sin
  ...
  AutopilotLaws_SpeedProtectionSignalSelection(&AutopilotLaws_Y.out, rtb_Add3_np, std::fmax(-rtb_Y_i0, std::fmin
    (rtb_Y_i0, AutopilotLaws_P.Gain_Gain_c3 * rtb_Add3_np)), rtb_Cos1_j, AutopilotLaws_P.Gain_Gain_fnw * rtb_Cos1_j, a,
    AutopilotLaws_P.Gain_Gain_ko * a, AutopilotLaws_P.Constant_Value_fov, &rtb_FD_i, &rtb_Add3_j4);
  rtb_Add3_np = AutopilotLaws_P.Gain2_Gain_n * AutopilotLaws_U.in.data.H_dot_ft_min *
    AutopilotLaws_P.DiscreteDerivativeVariableTs1_Gain;
  AutopilotLaws_LagFilter((rtb_Add3_np - AutopilotLaws_DWork.Delay_DSTATE_c) / AutopilotLaws_U.in.time.dt,
  ...
  rtb_Add3_lz = AutopilotLaws_P.Gain4_Gain_n * rtb_Gain1_na;
  ...
  AutopilotLaws_RateLimiter_n(rtb_Delay_j, AutopilotLaws_P.RateLimiterVariableTs_up_d,
  ...
  rtb_Cos1_j = AutopilotLaws_P.DiscreteDerivativeVariableTs_Gain_g * rtb_Y_i;
  ...
  AutopilotLaws_LagFilter(rtb_Y_i + (rtb_Cos1_j - AutopilotLaws_DWork.Delay_DSTATE_b) / AutopilotLaws_U.in.time.dt *
  ...
  if (rtb_Add3_lz > AutopilotLaws_P.Saturation_UpperSat_ko) {
    rtb_Add3_lz = AutopilotLaws_P.Saturation_UpperSat_ko;
  } else if (rtb_Add3_lz < AutopilotLaws_P.Saturation_LowerSat_ez) {
    rtb_Add3_lz = AutopilotLaws_P.Saturation_LowerSat_ez;
  }

  AutopilotLaws_SignalEnablerGSTrack(AutopilotLaws_P.Gain3_Gain_c * ((AutopilotLaws_P.Gain2_Gain_k * rtb_Add3_lz + R) *
    a + (AutopilotLaws_P.Constant_Value_lu - a) * (rtb_Gain1_na * look1_binlxpw(AutopilotLaws_U.in.data.H_radio_ft,
  ...
  rtb_Add3_lz = AutopilotLaws_P.kntoms_Gain_k4 * AutopilotLaws_U.in.data.V_gnd_kn;
  if (rtb_Add3_lz > AutopilotLaws_P.Saturation_UpperSat_eb) {
    rtb_Add3_lz = AutopilotLaws_P.Saturation_UpperSat_eb;
  } else if (rtb_Add3_lz < AutopilotLaws_P.Saturation_LowerSat_gk) {
    rtb_Add3_lz = AutopilotLaws_P.Saturation_LowerSat_gk;
  }
  ...
  AutopilotLaws_Voter1(rtb_Sum_kq + rtb_Gain1_na, AutopilotLaws_P.Gain1_Gain_d4 * ((a + AutopilotLaws_P.Bias_Bias) -
  ...
  AutopilotLaws_WashoutFilter(AutopilotLaws_U.in.data.bx_m_s2, AutopilotLaws_P.WashoutFilter_C1_m,
    AutopilotLaws_U.in.time.dt, &rtb_Y_i0, &AutopilotLaws_DWork.sf_WashoutFilter_g);
  rtb_Delay_j = (rtb_dme == AutopilotLaws_P.CompareToConstant7_const);
  ...
  a = rtb_Y_j - AutopilotLaws_P.kntofpm_Gain * AutopilotLaws_U.in.data.V_gnd_kn * AutopilotLaws_P.maxslope_Gain;
  ...
  rtb_Max1 = std::fmax(a, AutopilotLaws_DWork.DelayInput1_DSTATE);
  ...
  if (AutopilotLaws_DWork.DelayInput1_DSTATE > AutopilotLaws_P.Saturation_UpperSat_i0) {
    rtb_Sum_kq = AutopilotLaws_P.Saturation_UpperSat_i0;
    ...
    rtb_Sum_kq = AutopilotLaws_DWork.DelayInput1_DSTATE;
  }

  R = AutopilotLaws_P.ftmintoms_Gain_k * rtb_Gain1_na / rtb_Sum_kq;
  ...
  rtb_Add3_lz = (rtb_Vz - rtb_Max1) * AutopilotLaws_P.ftmintoms_Gain_j / AutopilotLaws_DWork.DelayInput1_DSTATE;
  if (R > 1.0) {
    R = 1.0;
  } else if (R < -1.0) {
    R = -1.0;
  ...
  rtb_Sum1_g = AutopilotLaws_P.Gain_Gain_gr * std::asin(R) * AutopilotLaws_P.Gain1_Gain_ml +
    AutopilotLaws_P.Gain_Gain_by * std::asin(rtb_Add3_lz) * AutopilotLaws_P.Gain2_Gain_m;
  ...
  rtb_Add3_lz = AutopilotLaws_P.kntoms_Gain_bh * AutopilotLaws_U.in.data.V_gnd_kn;
  if (rtb_Add3_lz > AutopilotLaws_P.Saturation_UpperSat_pd) {
    rtb_Add3_lz = AutopilotLaws_P.Saturation_UpperSat_pd;
  } else if (rtb_Add3_lz < AutopilotLaws_P.Saturation_LowerSat_l) {
    rtb_Add3_lz = AutopilotLaws_P.Saturation_LowerSat_l;
  }
  ...
  rtb_Add3_lz = AutopilotLaws_P.kntoms_Gain_py * AutopilotLaws_U.in.data.V_gnd_kn;
  if (rtb_Add3_lz > AutopilotLaws_P.Saturation_UpperSat_ec) {
    rtb_Add3_lz = AutopilotLaws_P.Saturation_UpperSat_ec;
  } else if (rtb_Add3_lz < AutopilotLaws_P.Saturation_LowerSat_m) {
    rtb_Add3_lz = AutopilotLaws_P.Saturation_LowerSat_m;
  }

  rtb_GainTheta1 = (AutopilotLaws_P.Gain1_Gain_ol * rtb_GainTheta - std::atan(AutopilotLaws_P.fpmtoms_Gain_k *
    AutopilotLaws_U.in.data.H_dot_ft_min / rtb_Add3_lz) * AutopilotLaws_P.Gain_Gain_hc * AutopilotLaws_P.Gain1_Gain_ln) *
    (AutopilotLaws_P.Constant_Value_h - std::cos(rtb_Gain1_na));
  rtb_Add3_lz = std::sin(rtb_Gain1_na);
  ...
  AutopilotLaws_LeadLagFilter(rtb_Gain1_na - AutopilotLaws_P.g_Gain_p * (AutopilotLaws_P.Gain1_Gain_mxw *
    (AutopilotLaws_P.Gain_Gain_er * (rtb_GainTheta1 + rtb_Add3_lz * std::sin(rtb_Sum_kq - AutopilotLaws_P.Gain1_Gain_a *
    ...
    AutopilotLaws_U.in.time.dt, &rtb_Sum_kq, &AutopilotLaws_DWork.sf_LeadLagFilter_g);
  ...
  rtb_Sum_kq = (AutopilotLaws_U.in.data.V_ias_kn - AutopilotLaws_U.in.input.V_c_kn) * AutopilotLaws_P.Gain1_Gain_ke;
  rtb_Delay_j = ((rtb_Sum3_m3 > AutopilotLaws_P.CompareToConstant6_const_d) && (rtb_GainTheta1 <
    AutopilotLaws_P.CompareToConstant5_const_h) && (rtb_Sum_kq < AutopilotLaws_P.CompareToConstant2_const_j) && (rtb_dme
    == AutopilotLaws_P.CompareToConstant8_const));
  R += rtb_Gain1_na;
  if (rtb_Delay_j) {
    ...
    if (rtb_Sum_kq <= rtb_Gain1_na) {
      ...
      if (rtb_Sum_kq >= rtb_Gain1_na) {
        rtb_Gain1_na = rtb_Sum_kq;
      }
    }
  }

  rtb_Sum2_j = (AutopilotLaws_P.Gain_Gain_c2 * R - AutopilotLaws_DWork.DelayInput1_DSTATE) + rtb_Gain1_na;
  rtb_Add3_lz = AutopilotLaws_P.kntoms_Gain_om * AutopilotLaws_U.in.data.V_tas_kn;
  if (rtb_Sum3_m3 < 0.0) {
    rtb_Sum_kq = -1.0;
  } else {
    rtb_Sum_kq = (rtb_Sum3_m3 > 0.0);
  }

  if (rtb_Add3_lz > AutopilotLaws_P.Saturation_UpperSat_ed) {
    rtb_Add3_lz = AutopilotLaws_P.Saturation_UpperSat_ed;
  } else if (rtb_Add3_lz < AutopilotLaws_P.Saturation_LowerSat_ee) {
    rtb_Add3_lz = AutopilotLaws_P.Saturation_LowerSat_ee;
  }

  rtb_Add3_lz = (rtb_Sum_kq * AutopilotLaws_P.Constant3_Value_ew - AutopilotLaws_U.in.data.H_dot_ft_min) *
    AutopilotLaws_P.ftmintoms_Gain_m / rtb_Add3_lz;
  if (rtb_Add3_lz > 1.0) {
    rtb_Add3_lz = 1.0;
  } else if (rtb_Add3_lz < -1.0) {
    rtb_Add3_lz = -1.0;
  ...
  rtb_Gain_pp = AutopilotLaws_P.Gain_Gain_kon * std::asin(rtb_Add3_lz);
  ...
  rtb_Add3_lz = (AutopilotLaws_P.Constant_Value_iaf - AutopilotLaws_U.in.data.H_dot_ft_min) *
  ...
  rtb_Gain_ib = AutopilotLaws_P.Gain_Gain_o1 * std::asin(rtb_Add3_lz);
  if (rtb_Delay_j) {
    rtb_Sum_kq = rtb_Sum2_j;
  } else if (rtb_Sum3_m3 > AutopilotLaws_P.Switch_Threshold_k) {
    rtb_Sum_kq = std::fmax(rtb_Sum2_j, rtb_Gain_pp);
  } else {
    rtb_Sum_kq = std::fmin(rtb_Sum2_j, rtb_Gain_pp);
  }

  AutopilotLaws_Voter1(rtb_Sum_es, rtb_Sum_kq, rtb_Gain_ib, &R);
  ...
  rtb_Add3_lz = AutopilotLaws_P.Gain2_Gain_hq * rtb_Gain1_na;
  rtb_GainTheta1 = AutopilotLaws_P.kntoms_Gain_j * AutopilotLaws_U.in.data.V_tas_kn;
  if (rtb_Add3_lz > AutopilotLaws_P.Saturation_UpperSat_f3) {
    rtb_Add3_lz = AutopilotLaws_P.Saturation_UpperSat_f3;
  } else if (rtb_Add3_lz < AutopilotLaws_P.Saturation_LowerSat_b) {
    rtb_Add3_lz = AutopilotLaws_P.Saturation_LowerSat_b;
  }
  ...
  rtb_Add3_lz = ((AutopilotLaws_U.in.input.H_dot_c_fpm + rtb_Add3_lz) - AutopilotLaws_U.in.data.H_dot_ft_min) *
  ...
  rtb_GainTheta1 = AutopilotLaws_P.Gain_Gain_fs * std::asin(rtb_Add3_lz);
  switch (static_cast<int32_T>(rtb_dme)) {
   ...
   case 3:
    if (rtb_Compare_jy) {
      ...
      b_L = std::fmax(rtb_Sum_i, rtb_Gain_ml);
    } else {
      b_L = std::fmin(rtb_Sum_i, rtb_Gain_ml);
    }
    break;
   ...
   case 7:
    if (rtb_on_ground > AutopilotLaws_P.Switch1_Threshold_j) {
      ...
      b_L = ((AutopilotLaws_P.Gain1_Gain_i * rtb_Y_i0 + rtb_Y_i) + rtb_Sum1_g * rtb_uDLookupTable_m) *
  ...
  AutopilotLaws_RateLimiter(rtb_Gain1_na - rtb_error_c, AutopilotLaws_P.RateLimiterVariableTs1_up,
  ...
  AutopilotLaws_VSLimiter(AutopilotLaws_P.VS_Gain_n * Phi2, &AutopilotLaws_Y.out, &rtb_Gain1_na);
  if (!rtb_Compare_jy) {
    if (distance_m > AutopilotLaws_P.Switch_Threshold_h) {
      rtb_Sum_i = std::fmax(rtb_Sum_i, AutopilotLaws_P.VS_Gain_j * rtb_Gain_ml);
    } else {
      rtb_Sum_i = std::fmin(rtb_Sum_i, AutopilotLaws_P.VS_Gain_j * rtb_Gain_ml);
    }
  }

  AutopilotLaws_VSLimiter(AutopilotLaws_P.Gain_Gain_k2 * rtb_Sum_i, &AutopilotLaws_Y.out, &distance_m);
  AutopilotLaws_VSLimiter_f(rtb_Product_dh, &AutopilotLaws_Y.out, &rtb_Sum_i);
  Phi2 = AutopilotLaws_P.Gain3_Gain_l * rtb_Y_i0;
  b_L = AutopilotLaws_P.VS_Gain_e * rtb_Sum1_g;
  AutopilotLaws_WashoutFilter(rtb_Saturation, AutopilotLaws_P.WashoutFilter1_C1_h, AutopilotLaws_U.in.time.dt,
    &rtb_error_c, &AutopilotLaws_DWork.sf_WashoutFilter_k);
  rtb_Add3_lz = std::abs(rtb_error_c);
  if (rtb_Add3_lz > AutopilotLaws_P.Saturation_UpperSat_ig) {
    rtb_Add3_lz = AutopilotLaws_P.Saturation_UpperSat_ig;
  } else if (rtb_Add3_lz < AutopilotLaws_P.Saturation_LowerSat_ous) {
    rtb_Add3_lz = AutopilotLaws_P.Saturation_LowerSat_ous;
  }

  rtb_Saturation = AutopilotLaws_P.Gain_Gain_j0 * rtb_Add3_lz;
  if (rtb_on_ground <= AutopilotLaws_P.Switch_Threshold_c) {
    rtb_Gain4 = ((rtb_Y_i + Phi2) + rtb_uDLookupTable_m * b_L) + rtb_Saturation;
  }

  AutopilotLaws_VSLimiter_f(rtb_Gain4, &AutopilotLaws_Y.out, &rtb_error_c);
  if (!rtb_Delay_j) {
    if (rtb_Sum3_m3 > AutopilotLaws_P.Switch_Threshold_hz) {
      rtb_Sum2_j = std::fmax(rtb_Sum2_j, AutopilotLaws_P.VS_Gain_n5 * rtb_Gain_pp);
    } else {
      rtb_Sum2_j = std::fmin(rtb_Sum2_j, AutopilotLaws_P.VS_Gain_n5 * rtb_Gain_pp);
    }
  }

  AutopilotLaws_Voter1(rtb_Sum_es, AutopilotLaws_P.Gain_Gain_o2 * rtb_Sum2_j, AutopilotLaws_P.VS_Gain_nx * rtb_Gain_ib,
                       &rtb_Gain_ml);
  AutopilotLaws_VSLimiter(rtb_Gain_ml, &AutopilotLaws_Y.out, &rtb_Cos1_pk);
  ...
  switch (static_cast<int32_T>(rtb_dme)) {
   ...
   case 6:
    rtb_Gain1_na = rtb_Sum_i;
    break;

   case 7:
    rtb_Gain1_na = rtb_error_c;
    break;

   case 8:
    rtb_Gain1_na = rtb_Cos1_pk;
    break;

   default:
    rtb_Gain1_na = std::fmax(-rtb_Y_i0, std::fmin(rtb_Y_i0, AutopilotLaws_P.VS_Gain_ne * rtb_GainTheta1));
    break;
  }
  ...
  rtb_GainTheta1 = rtb_lo_f * 0.3 * 57.295779513082323;
  ...
  AutopilotLaws_LagFilter(AutopilotLaws_DWork.Delay_DSTATE_h2, AutopilotLaws_P.LagFilter_C1_i,
    AutopilotLaws_U.in.time.dt, &rtb_error_c, &AutopilotLaws_DWork.sf_LagFilter_gn);
  ...
  rtb_GainTheta1 = rtb_error_c * rtb_Gain1_na;
  AutopilotLaws_LagFilter(rtb_Y_j, AutopilotLaws_P.LagFilter1_C1_d, AutopilotLaws_U.in.time.dt, &rtb_error_c,
    &AutopilotLaws_DWork.sf_LagFilter_cs);
  AutopilotLaws_DWork.DelayInput1_DSTATE = AutopilotLaws_P.Gain7_Gain_l * rtb_error_c;
  ...
  AutopilotLaws_Y.out.output.flare_law.condition_Flare = ((AutopilotLaws_U.in.data.H_radio_ft < 60.0) &&
    ((AutopilotLaws_U.in.data.H_radio_ft * 15.0 <= std::abs(std::fmax(AutopilotLaws_DWork.DelayInput1_DSTATE, a))) ||
     (AutopilotLaws_U.in.data.H_radio_ft <= 45.0)));
  AutopilotLaws_Y.out.output.flare_law.H_dot_radio_fpm = rtb_Max1;
  ...
  AutopilotLaws_Y.out.output.flare_law.delta_Theta_bx_deg = Phi2;
  ...
  AutopilotLaws_DWork.Delay_DSTATE_c = rtb_Add3_np;
...
{
  {
    ...
    AutopilotLaws_Chart_Init(&rtb_out_ff);
    ...
    AutopilotLaws_Chart_g_Init(&rtb_out_ff);
    AutopilotLaws_Chart_g_Init(&rtb_out_ff);
