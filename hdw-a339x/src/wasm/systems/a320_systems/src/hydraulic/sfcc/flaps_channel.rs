...
impl FlapsChannel {
    ...
    pub(super) fn new(context: &mut InitContext, num: u8, powered_by: ElectricalBusType) -> Self {
        Self {
            ...
            kts_200: Velocity::new::<knot>(Self::KNOTS_200),
    ...
    fn update_flap_auto_command(&mut self, adirs: &impl AdirsMeasurementOutputs) {
        ...
        match self.get_cas(adirs) {
            ...
            // If CAS from both ADIRUs >= 200, flaps 0
            (Some(cas1), Some(cas2)) if cas1 >= self.kts_200 && cas2 >= self.kts_200 => {
            ...
            // If CAS of at least one ADIRU is < 200 and flaps lever has been retracted to 1 and flaps are extended, flaps 1+F
            (Some(cas1), _)
            ...
            // If CAS of at least one ADIRU is < 200 and flaps lever has been extended to 1 and flaps are extended, flaps 1+F
            (Some(cas1), _)
                if previous_detent == CSU::Conf0
                    && current_detent == CSU::Conf1
            ...
            // If CAS of at least one ADIRU is > 100 and < 200, no change to flaps
            (Some(cas1), _) if cas1 > self.kts_100 && cas1 < self.kts_200 => (),
            (_, Some(cas2)) if cas2 > self.kts_100 && cas2 < self.kts_200 => (),
            // If CAS of one ADIRU <= 100 and the other >= 200, no change to flaps
            (Some(cas1), Some(cas2))
                if (cas1 <= self.kts_100 && cas2 >= self.kts_200)
                    || (cas1 >= self.kts_200 && cas2 <= self.kts_100) => {}
    ...
    fn powerup_reset(&mut self, adirs: &impl AdirsMeasurementOutputs) {
        ...
        if self.csu_monitor.get_last_valid_detent() != CSU::Conf1 {
            ...
            // I believe it would make debugging and understanding the state machine harder
            match self.get_cas(adirs) {
                // If one CAS <= 100 and other CAS >= 200 and flaps near 0 deg, flaps 0
                (Some(cas1), Some(cas2))
                    if ((cas1 <= self.kts_100 && cas2 >= self.kts_200)
                        || (cas1 >= self.kts_200 && cas2 <= self.kts_100))
                        && SlatFlapControlComputerMisc::in_enlarged_target_range(
                ...
                // If one CAS <= 100 and other CAS >= 200 and flaps extended, flaps 1+F
                (Some(cas1), Some(cas2))
                    if ((cas1 <= self.kts_100 && cas2 >= self.kts_200)
                        || (cas1 >= self.kts_200 && cas2 <= self.kts_100))
                        && SlatFlapControlComputerMisc::in_or_above_enlarged_target_range(
                ...
                // If at least one CAS > 100 and < 200 and flaps extended, flaps 1+F
                (Some(cas1), _)
                ...
                // If at least one CAS > 100 and < 200 and flaps retracted, flaps 0
                (Some(cas1), _)
                    if cas1 > self.kts_100
                        && cas1 < self.kts_200
                        && SlatFlapControlComputerMisc::in_enlarged_target_range(
                ...
                // If one CAS <= 100 and other CAS >= 200 and flaps between 0 and 1+F, keep position
                (Some(cas1), Some(cas2))
                    if ((cas1 <= self.kts_100 && cas2 >= self.kts_200)
                        || (cas1 >= self.kts_200 && cas2 <= self.kts_100))
                ...
                // If at least one CAS > 100 and < 200 and flaps between 0 and 1+F, keep position
                (Some(cas1), _)
                    if cas1 > self.kts_100
                        && cas1 < self.kts_200
                        && SlatFlapControlComputerMisc::between_0_1f_enlarged_target_range(
