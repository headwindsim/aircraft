...
impl A330Pneumatic {
    ...
    pub(crate) fn update(
        ...
        overhead_panel: &A330PneumaticOverheadPanel,
...
impl PneumaticBleed for A330Pneumatic {
...
impl EngineStartState for A330Pneumatic {
...
impl PackFlowValveState for A330Pneumatic {
    // pack_id: 1 or 2
...
impl SimulationElement for A330Pneumatic {
...
impl ReservoirAirPressure for A330Pneumatic {
...
pub struct A330PneumaticOverheadPanel {
...
impl A330PneumaticOverheadPanel {
    pub fn new(context: &mut InitContext) -> Self {
        A330PneumaticOverheadPanel {
...
impl EngineBleedPushbutton<2> for A330PneumaticOverheadPanel {
    fn engine_bleed_pushbuttons_are_auto(&self) -> [bool; 2] {
        [self.engine_1_bleed.is_auto(), self.engine_2_bleed.is_auto()]
    }
}
impl SimulationElement for A330PneumaticOverheadPanel {
    fn accept<T: SimulationElementVisitor>(&mut self, visitor: &mut T) {
...
/// This is a unique valve (and specific to the A330 probably) because it is controlled by two motors. One for manual control and one for automatic control
...
pub mod tests {
    ...
    use crate::air_conditioning::{A330AirConditioningSystem, A330PressurizationOverheadPanel};

    use super::{A330Pneumatic, A330PneumaticOverheadPanel};
    ...
    struct PneumaticTestAircraft {
        ...
        engine_1: LeapEngine,
        engine_2: LeapEngine,
        pneumatic_overhead_panel: A330PneumaticOverheadPanel,
    ...
    impl PneumaticTestAircraft {
        fn new(context: &mut InitContext) -> Self {
            Self {
                ...
                pneumatic_overhead_panel: A330PneumaticOverheadPanel::new(context),
    ...
    fn full_graphing_test() {
        ...
        if fs::create_dir_all("../A330_pneumatic_simulation_graphs/out").is_ok() {
            let mut file = File::create("../A330_pneumatic_simulation_graphs/out/generic_data.txt")
    ...
    fn hydraulic_reservoir_pressurization_graphs() {
        ...
        if fs::create_dir_all("../A330_pneumatic_simulation_graphs/out").is_ok() {
            let mut file = File::create(
                "../A330_pneumatic_simulation_graphs/out/hydraulic_reservoir_pressures_data.txt",
            )
            .expect("Could not create file");
    ...
    fn pack_pressurization_graphs() {
        ...
        if fs::create_dir_all("../A330_pneumatic_simulation_graphs/out").is_ok() {
            let mut file =
                File::create("../A330_pneumatic_simulation_graphs/out/pack_pressures_data.txt")
                    .expect("Could not create file");
