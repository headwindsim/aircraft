...
impl BoardingTestBed {
    ...
    fn init_vars(mut self) -> Self {
        ...
        self.write_by_name("WB_PER_PAX_WEIGHT", A330Payload::DEFAULT_PER_PAX_WEIGHT_KG);
    ...
    fn board_gsx_pax_half(mut self) -> Self {
        ...
        for ps in 0..A330Payload::A330_PAX.len() {
    ...
    fn load_pax(&mut self, ps: usize, pax_qty: i8) {
        ...
        self.write_by_name(A330Payload::A330_PAX[ps].pax_id, pax_flag);
        self.write_by_name(A330Payload::A330_PAX[ps].payload_id, payload);
    }
    ...
    fn target_pax(&mut self, ps: usize, pax_qty: i8) {
        ...
        self.write_by_name(
            &format!("{}_DESIRED", A330Payload::A330_PAX[ps].pax_id),
            pax_flag,
        );
    }
    ...
    fn load_cargo(&mut self, cs: usize, cargo_qty: Mass) {
        ...
        self.write_by_name(
            A330Payload::A330_CARGO[cs].cargo_id,
            cargo_qty.get::<kilogram>(),
        );
        self.write_by_name(
            A330Payload::A330_CARGO[cs].payload_id,
            cargo_qty.get::<pound>(),
        );
    }
    ...
    fn target_cargo(&mut self, cs: usize, cargo_qty: Mass) {
        ...
        self.write_by_name(
            &format!("{}_DESIRED", A330Payload::A330_CARGO[cs].cargo_id),
            cargo_qty.get::<kilogram>(),
        );
    }
    ...
    fn with_all_stations_half_cargo(mut self) -> Self {
        for cs in 0..A330Payload::A330_CARGO.len() {
...
fn boarding_init() {
    ...
    assert!(test_bed.contains_variable_with_name(
        A330Payload::A330_PAX[Into::<usize>::into(A330Pax::A)].pax_id
    ));
    assert!(test_bed.contains_variable_with_name(
        A330Payload::A330_PAX[Into::<usize>::into(A330Pax::B)].pax_id
    ));
    assert!(test_bed.contains_variable_with_name(
        A330Payload::A330_PAX[Into::<usize>::into(A330Pax::C)].pax_id
    ));
    assert!(test_bed.contains_variable_with_name(
        A330Payload::A330_PAX[Into::<usize>::into(A330Pax::D)].pax_id
    ));
}
#[test]
fn loaded_no_pax() {
...
fn detailed_test_with_multiple_stops() {
    let mut test_bed = test_bed_with()
        .init_vars()
        .with_pax(A330Pax::A.into(), 5)
        .with_pax(A330Pax::B.into(), 1)
        .with_pax(A330Pax::C.into(), 16)
        .with_pax(A330Pax::D.into(), 42)
        .with_pax_target(A330Pax::A.into(), 15)
        .with_pax_target(A330Pax::B.into(), 14)
        .with_pax_target(A330Pax::C.into(), 32)
        .with_pax_target(A330Pax::D.into(), 12)
        .with_cargo(A330Cargo::FwdBaggage.into(), Mass::new::<kilogram>(100.0))
        .with_cargo(A330Cargo::AftBaggage.into(), Mass::new::<kilogram>(100.0))
        .with_cargo(A330Cargo::AftBulkLoose.into(), Mass::new::<kilogram>(100.0))
        .with_cargo(A330Cargo::AftContainer.into(), Mass::new::<kilogram>(100.0))
        .real_board_rate()
        .start_boarding()
        .and_run()
        .and_stabilize();
    ...
    assert_eq!(test_bed.pax_num(A330Pax::A.into()), 15);
    assert_eq!(test_bed.pax_num(A330Pax::B.into()), 14);
    assert_eq!(test_bed.pax_num(A330Pax::C.into()), 32);
    assert_eq!(test_bed.pax_num(A330Pax::D.into()), 34);
    ...
    assert_eq!(test_bed.pax_num(A330Pax::B.into()), 14);
    assert_eq!(test_bed.pax_num(A330Pax::C.into()), 32);
    assert_eq!(test_bed.pax_num(A330Pax::D.into()), 12);
    test_bed.has_no_cargo();

    test_bed = test_bed
        .init_vars()
        .with_pax_target(A330Pax::A.into(), 0)
        .with_pax_target(A330Pax::B.into(), 0)
        .with_pax_target(A330Pax::C.into(), 0)
        .with_pax_target(A330Pax::D.into(), 0)
        .target_half_cargo()
        .instant_board_rate()
        .start_boarding()
        .and_run()
...
fn gsx_deboarding_initial_state() {
    let mut test_bed = test_bed_with()
        ...
        .init_vars_gsx()
        .with_full_cargo()
        .with_pax(A330Pax::A.into(), 25)
        .with_pax(A330Pax::B.into(), 25)
        .with_pax(A330Pax::C.into(), 25)
        .with_pax(A330Pax::D.into(), 25)
        .target_no_pax()
        .target_no_cargo()
