...
impl PackFlowControllers for A330AirConditioning {
...
struct A330Cabin {
    ...
    cabin_air_simulation: CabinAirSimulation<A330PressurizationConstants, 3>,
}
...
impl A330Cabin {
    ...
    fn update(
        ...
        lgciu: [&impl LgciuWeightOnWheels; 2],
        number_of_passengers: &impl NumberOfPassengers,
        pressurization: &A330PressurizationSystem,
    ) {
    ...
    fn update_number_of_passengers(&mut self, number_of_passengers: &impl NumberOfPassengers) {
        self.number_of_passengers[1] =
            (number_of_passengers.number_of_passengers(A330Pax::A.into())
                + number_of_passengers.number_of_passengers(A330Pax::B.into())
                + number_of_passengers.number_of_passengers(A330Pax::C.into())
                + number_of_passengers.number_of_passengers(A330Pax::D.into())
                + number_of_passengers.number_of_passengers(A330Pax::E.into())) as u8;
        self.number_of_passengers[2] =
            (number_of_passengers.number_of_passengers(A330Pax::F.into())
                + number_of_passengers.number_of_passengers(A330Pax::G.into())
                + number_of_passengers.number_of_passengers(A330Pax::H.into())
                + number_of_passengers.number_of_passengers(A330Pax::I.into())
                + number_of_passengers.number_of_passengers(A330Pax::J.into())) as u8;
    }
}

impl CabinSimulation for A330Cabin {
...
impl SimulationElement for A330Cabin {
...
pub struct A330AirConditioningSystem {
    ...
    air_conditioning_overhead: A330AirConditioningSystemOverhead<3>,
}

impl A330AirConditioningSystem {
    ...
    pub(crate) fn new(context: &mut InitContext, cabin_zones: &[ZoneType; 3]) -> Self {
        Self {
            ...
            air_conditioning_overhead: A330AirConditioningSystemOverhead::new(context, cabin_zones),
        }
    }
...
impl PackFlowControllers for A330AirConditioningSystem {
    type PackFlowControllerSignal =
...
impl DuctTemperature for A330AirConditioningSystem {
    fn duct_temperature(&self) -> Vec<ThermodynamicTemperature> {
        self.trim_air_system.duct_temperature()
    }
}

impl OutletAir for A330AirConditioningSystem {
...
// This is not used in the A330
impl VcmShared for A330AirConditioningSystem {}

impl SimulationElement for A330AirConditioningSystem {
...
pub(crate) struct A330AirConditioningSystemOverhead<const ZONES: usize> {
...
impl<const ZONES: usize> A330AirConditioningSystemOverhead<ZONES> {
...
impl<const ZONES: usize> AirConditioningOverheadShared
    for A330AirConditioningSystemOverhead<ZONES>
{
...
impl<const ZONES: usize> SimulationElement for A330AirConditioningSystemOverhead<ZONES> {
    fn read(&mut self, reader: &mut SimulatorReader) {
...
struct A330PressurizationSystem {
    active_cpc_sys_id: VariableIdentifier,

    cpc: [CabinPressureController<A330PressurizationConstants>; 2],
...
impl A330PressurizationSystem {
    ...
    pub fn update(
        ...
        context: &UpdateContext,
        adirs: &impl AdirsToAirCondInterface,
        press_overhead: &A330PressurizationOverheadPanel,
        engines: [&impl EngineCorrectedN1; 2],
        lgciu: [&impl LgciuWeightOnWheels; 2],
        cabin_simulation: &impl CabinSimulation,
    ) {
...
impl CabinAltitude for A330PressurizationSystem {
    fn altitude(&self) -> Length {
        self.cpc[self.active_system - 1].cabin_altitude()
    }
}

impl SimulationElement for A330PressurizationSystem {
    fn write(&self, writer: &mut SimulatorWriter) {
...
struct A330PressurizationConstants;

impl PressurizationConstants for A330PressurizationConstants {
...
pub struct A330PressurizationOverheadPanel {
...
impl A330PressurizationOverheadPanel {
    pub fn new(context: &mut InitContext) -> Self {
        Self {
...
impl SimulationElement for A330PressurizationOverheadPanel {
    fn accept<T: SimulationElementVisitor>(&mut self, visitor: &mut T) {
...
impl ControllerSignal<OutflowValveSignal> for A330PressurizationOverheadPanel {
...
impl PressurizationOverheadShared for A330PressurizationOverheadPanel {
        ...
        fn when_on_ground_pressure_diff_is_less_than_excessive() {
            ...
            assert!(
                test_bed.cabin_delta_p()
                    < Pressure::new::<psi>(
                        A330PressurizationConstants::EXCESSIVE_RESIDUAL_PRESSURE_WARNING
                    )
            );
        }
