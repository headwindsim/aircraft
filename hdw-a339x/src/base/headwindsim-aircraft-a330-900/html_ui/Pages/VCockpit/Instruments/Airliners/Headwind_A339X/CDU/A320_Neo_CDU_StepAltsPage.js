class CDUStepAltsPage {
  ...
  static ShowPage(mcdu) {
    ...
    const isFlying =
      mcdu.flightPhaseManager.phase >= FmgcFlightPhases.TAKEOFF &&
      mcdu.flightPhaseManager.phase < FmgcFlightPhases.DONE;
    const transitionAltitude = mcdu.flightPlanManager.originTransitionAltitude;

    const predictions =
      mcdu.guidanceController.vnavDriver.mcduProfile && mcdu.guidanceController.vnavDriver.mcduProfile.isReadyToDisplay
    ...
    for (let i = 0; i < 4; i++) {
      mcdu.onLeftInput[i] = (value, scratchpadCallback) =>
        CDUStepAltsPage.tryAddOrUpdateCruiseStepFromLeftInput(mcdu, scratchpadCallback, steps, i, value);
    }
  ...
  static tryParseAltitude(altitudeInput) {
    ...
    if (altValue < 1000 || altValue > 41000) {
      return false;
    }
  ...
  static onClickExistingStepClimb(mcdu, scratchpadCallback, stepWaypoints, index, input) {
    ...
    if (splitInputs.length === 1 || (splitInputs.length === 2 && splitInputs[1] === '')) {
      ...
      if (!altitude) {
        ...
        !this.checkStepInsertionRules(mcdu, stepWaypoints, clickedStep.wapyointIndex, clickedStep.toAltitude)
      ) {
        // Step too small or step descent after step climb
        mcdu.setScratchpadMessage(NXSystemMessages.notAllowed);
        scratchpadCallback();
