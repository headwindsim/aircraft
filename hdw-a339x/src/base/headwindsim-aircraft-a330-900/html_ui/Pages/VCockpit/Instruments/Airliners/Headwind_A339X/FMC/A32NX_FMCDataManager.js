...
class FMCDataManager {
  constructor(_fmc) {
      ...
      JSON.parse(stored).forEach((wp) => {
        if (wp) {
          this.storedWaypoints.push(
            Fmgc.WaypointBuilder.fromCoordinates(wp.ident, new LatLongAlt(wp.coordinates), this.fmc, wp.additionalData),
          );
        } else {
    ...
    NXDataStore.getAndSubscribe('LATLON_EXT_FMT', (_, value) => (this.latLonExtendedFormat = value === '1'), '1');
  }
  ...
  async GetWaypointsByIdentAndType(ident, filter = 0, maxItems = 40) {
    ...
    const results = (await Coherent.call('SEARCH_BY_IDENT', ident, filter, maxItems)).filter(
      (icao) => ident === icao.substr(7, 5).trim(),
    );
    const waypoints = (
      await Promise.all(results.map(async (icao) => await this.fmc.facilityLoader.getFacility(icao)))
  ...
  _updateLocalStorage() {
    localStorage.setItem(
      FMCDataManager.STORED_WP_KEY,
      JSON.stringify(
        this.storedWaypoints.map((wp) =>
          wp
            ? {
                ident: wp.ident,
  ...
  createPlaceBearingPlaceBearingWaypoint(place1, bearing1, place2, bearing2, stored = false, ident = undefined) {
    const coordinates = A32NX_Util.greatCircleIntersection(
      place1.infos.coordinates,
      bearing1,
      place2.infos.coordinates,
      bearing2,
    );
  ...
  createPlaceBearingDistWaypoint(origin, bearing, distance, stored = false, ident = undefined) {
    const coordinates = Avionics.Utils.bearingDistanceToCoordinates(
      bearing,
      distance,
      origin.infos.coordinates.lat,
      origin.infos.coordinates.long,
    );
