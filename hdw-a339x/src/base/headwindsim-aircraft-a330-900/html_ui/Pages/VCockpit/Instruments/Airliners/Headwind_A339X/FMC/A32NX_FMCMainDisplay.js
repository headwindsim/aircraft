...
class FMCMainDisplay extends BaseAirliners {
    ...
    this.flightPlanManager.onCurrentGameFlightLoaded(() => {
      this.flightPlanManager.updateFlightPlan(() => {
        ...
        const callback = () => {
          ...
          console.log(
            'FlightPlan Cruise Override. Cruising at FL' +
              cruiseAlt +
              ' instead of default FL' +
    ...
    switch (nextPhase) {
      case FmgcFlightPhases.TAKEOFF: {
        ...
        if (plan.accelerationAltitude === undefined) {
          ...
          plan.accelerationAltitudePilot =
        ...
        if (plan.engineOutAccelerationAltitude === undefined) {
          // it's important to set this immediately as we don't want to immediately sequence to the climb phase
          plan.engineOutAccelerationAltitudePilot =
            SimVar.GetSimVarValue('INDICATED ALTITUDE', 'feet') + parseInt(NXDataStore.get('CONFIG_ACCEL_ALT', '1500'));
          this.updateThrustReductionAcceleration();
        }
      ...
      case FmgcFlightPhases.GOAROUND: {
        ...
        if (plan.missedAccelerationAltitude === undefined) {
          // it's important to set this immediately as we don't want to immediately sequence to the climb phase
          plan.missedAccelerationAltitudePilot =
        ...
        if (plan.missedEngineOutAccelerationAltitude === undefined) {
          // it's important to set this immediately as we don't want to immediately sequence to the climb phase
          plan.missedEngineOutAccelerationAltitudePilot =
            SimVar.GetSimVarValue('INDICATED ALTITUDE', 'feet') +
            parseInt(NXDataStore.get('CONFIG_ENG_OUT_ACCEL_ALT', '1500'));
          this.updateThrustReductionAcceleration();
        }
  ...
  getHoldingSpeed(speedConstraint = undefined, altitude = undefined) {
    ...
    if (alt < 14000) {
      ...
      kcas = Math.min(_convertMachToKCas(0.83, temperature, pressure), kcas);
    }
    ...
    if (SimVar.GetSimVarValue('L:A32NX_FMA_EXPEDITE_MODE', 'number') === 1) {
      ...
      switch (this.flightPhaseManager.phase) {
        ...
        case FmgcFlightPhases.GOAROUND: {
          ...
          const accAlt = engineOut
            ? activePlan.missedEngineOutAccelerationAltitude
            : activePlan.missedAccelerationAltitude;
  ...
  updateAutopilot() {
    ...
    if (this.updateAutopilotCooldown < 0) {
      ...
      if (
        this.flightPlanManager.isLoadedApproach() &&
        !this.flightPlanManager.isActiveApproach() &&
        (this.flightPlanManager.getActiveWaypointIndex() === -1 ||
          this.flightPlanManager.getActiveWaypointIndex() > this.flightPlanManager.getLastIndexBeforeApproach())
      ) {
  ...
  updateConstraints() {
    ...
    if (constraints) {
      if (
        (this.flightPhaseManager.phase < FmgcFlightPhases.CRUISE ||
          this.flightPhaseManager.phase === FmgcFlightPhases.GOAROUND) &&
  ...
  updateManagedProfile() {
    ...
    for (let index = this.flightPlanManager.getWaypointsCount(FlightPlans.Active) - 1; index >= 0; index--) {
      ...
      if (index <= this.flightPlanManager.getEnRouteWaypointsFirstIndex(FlightPlans.Active)) {
        ...
        wp.additionalData.predictedAltitude = Math.min(
          destinationElevation + 10000,
          wp.additionalData.predictedAltitude,
        );
        wp.additionalData.predictedSpeed = Math.min(250, wp.additionalData.predictedSpeed);
      }
    }
    ...
    if (this.landingElevation !== landingElevation) {
      ...
      const ssm =
        landingElevation !== undefined
          ? Arinc429Word.SignStatusMatrix.NormalOperation
          : Arinc429Word.SignStatusMatrix.NoComputedData;
  ...
  shouldTransmitMinimums() {
    ...
    return (
      phase > FmgcFlightPhases.CRUISE ||
      (phase === FmgcFlightPhases.CRUISE && this.flightPlanManager.getDistanceToDestination(FlightPlans.Active) < 250)
    );
  }
  ...
  _onStepClimbDescent() {
    if (
      !(
        this.flightPhaseManager.phase === FmgcFlightPhases.CLIMB ||
        this.flightPhaseManager.phase === FmgcFlightPhases.CRUISE
      )
    ) {
      return;
  ...
  async trySetRouteAlternateFuel(altFuel) {
    ...
    let alternateFlightLevel = 290;
    if (this.winds && this.winds.alternate && this.winds.alternate.altitude) {
      alternateFlightLevel = this.winds.alternate.altitude / 100;
    }
  ...
  tryUpdateRouteAlternate() {
    if (this._DistanceToAlt < 20) {
      ...
      const deviation =
        ...
        A32NX_FuelPred.computeNumbers(airDistance, alternateFlightLevel, A32NX_FuelPred.computations.CORRECTIONS, true);
      if (50 <= airDistance && airDistance <= 1200 && 100 <= alternateFlightLevel && alternateFlightLevel <= 390) {
        //This will always be true until we can setup alternate routes
        this._routeAltFuelWeight =
          (A32NX_FuelPred.computeNumbers(airDistance, alternateFlightLevel, A32NX_FuelPred.computations.FUEL, true) +
            deviation) /
          1000;
        this._routeAltFuelTime = this._routeAltFuelEntered
          ? null
          : A32NX_FuelPred.computeNumbers(airDistance, alternateFlightLevel, A32NX_FuelPred.computations.TIME, true);
      }
    }
  }
  ...
  setRunwayIndex(runwayIndex, callback = EmptyCallback.Boolean) {
    this.ensureCurrentFlightPlanIsTemporary(() => {
      ...
      if (!this.flightPlanManager.getOrigin()) {
        ...
        this.flightPlanManager
          .setDepartureRunwayIndex(-1, () => {
            this.flightPlanManager
              .setOriginRunwayIndex(-1, () => {
                return callback(true);
              })
              .catch(console.error);
          })
        ...
        if (routeOriginInfo.oneWayRunways[runwayIndex]) {
          this.flightPlanManager
            .setDepartureRunwayIndex(runwayIndex, () => {
              return callback(true);
            })
            .catch(console.error);
        }
  ...
  setDepartureIndex(departureIndex, callback = EmptyCallback.Boolean) {
    this.ensureCurrentFlightPlanIsTemporary(() => {
      ...
      this.flightPlanManager
        .setDepartureProcIndex(departureIndex, () => {
          if (currentRunway) {
            SimVar.SetSimVarValue(
              'L:A32NX_DEPARTURE_ELEVATION',
              'feet',
              A32NX_Util.meterToFeet(currentRunway.elevation),
            );
            const departure = this.flightPlanManager.getDeparture();
            const departureRunwayIndex = departure
              ? departure.runwayTransitions.findIndex((t) => {
                  return t.runwayNumber === currentRunway.number && t.runwayDesignation === currentRunway.designator;
                })
              : -1;
            if (departureRunwayIndex >= -1) {
              return this.flightPlanManager
                .setDepartureRunwayIndex(departureRunwayIndex, () => {
                  return callback(true);
                })
                .catch(console.error);
            }
  ...
  setArrivalIndex(arrivalIndex, transitionIndex, callback = EmptyCallback.Boolean) {
    ...
    this.ensureCurrentFlightPlanIsTemporary(() => {
      this.flightPlanManager
        .setArrivalEnRouteTransitionIndex(transitionIndex, () => {
          this.flightPlanManager
            .setArrivalProcIndex(arrivalIndex, () => {
              callback(true);
            })
  ...
  insertWaypoint(newWaypointTo, index, callback = EmptyCallback.Boolean, immediately) {
    ...
    try {
      this.getOrCreateWaypoint(newWaypointTo, true)
        .then((waypoint) => {
          if (!waypoint) {
            return callback(false);
          }
          if (immediately) {
            ...
            if (waypoint.additionalData && waypoint.additionalData.storedType !== undefined) {
              this.flightPlanManager
                .addUserWaypoint(waypoint, index, () => {
                  return callback(true);
                })
                .catch(console.error);
            } else {
              this.flightPlanManager
                .addWaypoint(waypoint.icao, index, () => {
                  return callback(true);
                })
                .catch(console.error);
            }
  ...
  async copyAirwaySelections() {
    ...
    for (let i = 0; i < activeFPWaypoints.length; i++) {
      if (
        activeFPWaypoints[i].infos &&
        temporaryFPWaypoints[i] &&
        activeFPWaypoints[i].icao === temporaryFPWaypoints[i].icao &&
        temporaryFPWaypoints[i].infos
      ) {
  ...
  thrustReductionAccelerationChecks() {
    ...
    if (activePlan.reconcileThrustReductionWithConstraints()) {
      this.addMessageToQueue(
        NXSystemMessages.newThrRedAlt.getModifiedMessage(activePlan.thrustReductionAltitude.toFixed(0)),
      );
    }
  }
  ...
  updateThrustReductionAcceleration() {
    ...
    this.arincThrustReductionAltitude.setBnrValue(
      ...
      activePlan.thrustReductionAltitude !== undefined
        ? Arinc429Word.SignStatusMatrix.NormalOperation
        : Arinc429Word.SignStatusMatrix.NoComputedData,
      17,
      131072,
    ...
    this.arincAccelerationAltitude.setBnrValue(
      ...
      activePlan.accelerationAltitude !== undefined
        ? Arinc429Word.SignStatusMatrix.NormalOperation
        : Arinc429Word.SignStatusMatrix.NoComputedData,
      17,
      131072,
    ...
    this.arincEoAccelerationAltitude.setBnrValue(
      ...
      activePlan.engineOutAccelerationAltitude !== undefined
        ? Arinc429Word.SignStatusMatrix.NormalOperation
        : Arinc429Word.SignStatusMatrix.NoComputedData,
      17,
      131072,
    ...
    this.arincMissedThrustReductionAltitude.setBnrValue(
      ...
      activePlan.missedThrustReductionAltitude !== undefined
        ? Arinc429Word.SignStatusMatrix.NormalOperation
        : Arinc429Word.SignStatusMatrix.NoComputedData,
      17,
      131072,
    ...
    this.arincMissedAccelerationAltitude.setBnrValue(
      ...
      activePlan.missedAccelerationAltitude !== undefined
        ? Arinc429Word.SignStatusMatrix.NormalOperation
        : Arinc429Word.SignStatusMatrix.NoComputedData,
      17,
      131072,
    ...
    this.arincMissedEoAccelerationAltitude.setBnrValue(
      ...
      activePlan.missedEngineOutAccelerationAltitude !== undefined
        ? Arinc429Word.SignStatusMatrix.NormalOperation
        : Arinc429Word.SignStatusMatrix.NoComputedData,
      17,
      131072,
  ...
  updateTransitionAltitudeLevel() {
    ...
    this.arincTransitionAltitude.setBnrValue(
      ...
      originTransitionAltitude !== undefined
        ? Arinc429Word.SignStatusMatrix.NormalOperation
        : Arinc429Word.SignStatusMatrix.NoComputedData,
      17,
      131072,
    ...
    this.arincTransitionLevel.setBnrValue(
      ...
      destinationTansitionLevel !== undefined
        ? Arinc429Word.SignStatusMatrix.NormalOperation
        : Arinc429Word.SignStatusMatrix.NoComputedData,
      9,
      512,
  ...
  trySetCruiseFl(fl) {
    ...
    if (
      fl < selFl &&
      (phase === FmgcFlightPhases.CLIMB || phase === FmgcFlightPhases.APPROACH || phase === FmgcFlightPhases.GOAROUND)
    ) {
      this.setScratchpadMessage(NXSystemMessages.entryOutOfRange);
      return false;
    }
    ...
    const curPerfPagePhase = (() => {
      switch (this.page.Current) {
        case this.page.PerformancePageTakeoff:
          return FmgcFlightPhases.TAKEOFF;
        case this.page.PerformancePageClb:
          return FmgcFlightPhases.CLIMB;
        case this.page.PerformancePageCrz:
          return FmgcFlightPhases.CRUISE;
        case this.page.PerformancePageDes:
          return FmgcFlightPhases.DESCENT;
        case this.page.PerformancePageAppr:
          return FmgcFlightPhases.APPROACH;
        case this.page.PerformancePageGoAround:
          return FmgcFlightPhases.GOAROUND;
      }
    })();
  ...
  trySetProgWaypoint(s, callback = EmptyCallback.Boolean) {
    ...
    try {
      this.getOrCreateWaypoint(s, false)
        .then((wp) => {
  ...
  isFlying() {
    return (
      this.flightPhaseManager.phase >= FmgcFlightPhases.TAKEOFF && this.flightPhaseManager.phase < FmgcFlightPhases.DONE
    );
  }
  /**
