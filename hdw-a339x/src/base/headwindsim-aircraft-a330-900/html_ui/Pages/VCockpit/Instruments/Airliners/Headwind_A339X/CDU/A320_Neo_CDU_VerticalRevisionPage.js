...
class CDUVerticalRevisionPage {
  static ShowPage(
    ...
    if (waypointInfo instanceof WayPointInfo) {
      ...
      const transAltLevel =
        constraintType === WaypointConstraintType.DES
          ? mcdu.flightPlanManager.destinationTransitionLevel * 100
          : mcdu.flightPlanManager.originTransitionAltitude;
      ...
      if (isDestination) {
        ...
        const closeToDest =
          mcdu.flightPlanManager.getDestination() && mcdu.flightPlanManager.getDestination().liveDistanceTo <= 180;
        l4Title = '\xa0QNH';
        ...
        if (
          mcdu._cruiseEntered &&
          mcdu._cruiseFlightLevel &&
          (mcdu.flightPhaseManager.phase < FmgcFlightPhases.DESCENT ||
            mcdu.flightPhaseManager.phase > FmgcFlightPhases.GOAROUND)
        ) {
          r5Cell = 'STEP ALTS>';
        }
      ...
      mcdu.onLeftInput[2] = async (value, scratchpadCallback) => {
        if (value === FMCMainDisplay.clrValue) {
          mcdu.flightPlanManager.setWaypointSpeed(
            -1,
            wpIndex,
            () => {
      ...
      mcdu.onRightInput[2] = (value, scratchpadCallback) => {
        ...
        mcdu.flightPlanManager.setWaypointAltitude(
          altitude,
          wpIndex,
          () => {
            mcdu.updateConstraints();
      ...
      mcdu.onLeftInput[4] = () => {
        ...
        /* TODO: READD THIS LATER WITH FMSv2
  ...
  static setConstraints(mcdu, waypoint, verticalWaypoint, value, scratchpadCallback, offset = 0) {
    ...
    if (speed !== undefined) {
      mcdu.flightPlanManager.setWaypointSpeed(
        speed,
        mcdu.flightPlanManager.indexOfWaypoint(waypoint),
        () => {
          mcdu.guidanceController.vnavDriver.invalidateFlightPlanProfile();
