class CDUWindPage {
  ...
  static ShowCLBPage(mcdu, offset = 0) {
    ...
    const template = [
      ...
      [new Column(0, 'TRU WIND/ALT'), new Column(22, 'HISTORY', Column.right, Column.inop)],
  ...
  static ShowCRZPage(mcdu, offset = 0) {
    ...
    const template = [
      ...
      [new Column(0, 'TRU WIND/WPT'), new Column(22, 'CRZ FL', Column.right)],
  ...
  static ShowWPTPage(mcdu, ident, offset = 0) {
    ...
    const template = [
      [new Column(4, 'CRZ WIND'), new Column(13, 'AT', Column.small), new Column(16, ident, Column.green)],
      ...
      ['', new Column(22, 'PREV', Column.right)],
      ['', new Column(23, 'PHASE>', Column.right)],
      [new Column(0, 'SAT / ALT', Column.small), new Column(22, 'NEXT', Column.right)],
      [new Column(0, '[ ]°/[   ]', Column.inop), new Column(23, 'PHASE>', Column.right)],
      [''],
      [new Column(0, '<RETURN')],
    ];

    mcdu.setTemplate(
      FormatTemplate(CDUWindPage.ShowWinds(template, mcdu, 'WPT', CDUWindPage.ShowWPTPage, offset, 4, ident)),
    );
  ...
  static ShowDESPage(mcdu, offset = 0) {
    ...
    if (mcdu.winds.alternate != null) {
      alternateCellValue = new Column(
        0,
        `${CDUWindPage.FormatNumber(mcdu.winds.alternate.direction, 2)}°/${CDUWindPage.FormatNumber(mcdu.winds.alternate.speed, 2)}/FL${mcdu.winds.alternate.altitude / 100}`,
        Column.cyan,
      );
    }
    ...
    if (mcdu.uplinkWinds.alternate != null) {
      alternateCellValue = new Column(
        0,
        `${CDUWindPage.FormatNumber(mcdu.uplinkWinds.alternate.direction, 2)}°/${CDUWindPage.FormatNumber(mcdu.uplinkWinds.alternate.speed, 2)}/FL${mcdu.uplinkWinds.alternate.altitude / 100}`,
        Column.green,
      );
    }
      ...
      if (i < _max) {
        ...
        switch (phase) {
          case 'CRZ':
            row = new Column(
              0,
              `${CDUWindPage.FormatNumber(wind.direction, 2)}°/${CDUWindPage.FormatNumber(wind.speed, 2)}/${wind.ident}`,
              color,
            );
            break;
          case 'CLB':
          ...
          case 'WPT':
            row = new Column(
              0,
              `${CDUWindPage.FormatNumber(wind.direction, 2)}°/${CDUWindPage.FormatNumber(wind.speed, 2)}/FL${wind.altitude}`,
              color,
            );
            break;
        }
  ...
  static WindRequest(mcdu, _showPage = null) {
    getSimBriefOfp(
      ...
      () => {
        ...
        const isOneEngineRunning =
          SimVar.GetSimVarValue('L:A32NX_ENGINE_N1:1', 'Number') >= 15 ||
          SimVar.GetSimVarValue('L:A32NX_ENGINE_N1:2', 'Number') >= 15;
        let clbWind = [];
        let crzWind = [];
        let desWind = [];
        let altWind = null;
        ...
        setTimeout(
          () => {
            if (
              !isOneEngineRunning &&
              mcdu.winds.climb.length === 0 &&
              mcdu.winds.cruise.length === 0 &&
              mcdu.winds.des.length === 0 &&
              mcdu.winds.alternate == null
            ) {
            ...
            if (
              (_showPage && mcdu.page.Current === mcdu.page.ClimbWind) ||
              mcdu.page.Current === mcdu.page.CruiseWind ||
              mcdu.page.Current === mcdu.page.DescentWind ||
