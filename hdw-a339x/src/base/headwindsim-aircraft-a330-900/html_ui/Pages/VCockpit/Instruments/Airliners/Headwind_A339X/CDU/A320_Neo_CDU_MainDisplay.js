class A320_Neo_CDU_MainDisplay extends FMCMainDisplay {
  ...
  mcduServerClientEventHandler(event) {
    switch (event.type) {
      ...
      case 'error': {
        console.log(
          `[MCDU] Websocket connection to SimBridge error. (${SimBridgeClient.McduServerClient.url()}): ${event.get()}`,
        );
        break;
      }
      case 'message': {
  ...
  setTemplate(template, large = false) {
    ...
    SimVar.SetSimVarValue(
      'L:AIRLINER_MCDU_CURRENT_FPLN_WAYPOINT',
      'number',
      this.currentFlightPlanWaypointIndex,
    ).then();
  ...
  onBrightnessKey(side, sign) {
    ...
    SimVar.SetSimVarValue(
      ...
      'number',
      Math.max(this.MIN_BRIGHTNESS, Math.min(this.MAX_BRIGHTNESS, oldBrightness + sign * 0.2 * oldBrightness)),
    );
  }
  ...
  getDelayRouteChange() {
    if (this._zeroFuelWeightZFWCGEntered && this._blockFuelEntered) {
      return (
        Math.pow(this.flightPlanManager.getWaypointsCount(), 2) +
        this.flightPlanManager.getDestination().cumulativeDistanceInFP / 10 +
        Math.random() * 300
      );
    } else {
      return (
        300 +
        this.flightPlanManager.getWaypointsCount() * Math.random() +
        this.flightPlanManager.getDestination().cumulativeDistanceInFP * Math.random()
      );
    }
  }
  ...
  getDelayFuelPred() {
    return (
      225 * this.flightPlanManager.getWaypointsCount() +
      this.flightPlanManager.getDestination().cumulativeDistanceInFP / 2
    );
  }
