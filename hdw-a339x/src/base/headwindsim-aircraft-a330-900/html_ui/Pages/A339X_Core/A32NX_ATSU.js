...
const getSimBriefOfp = (mcdu, updateView, callback = () => {}) => {
  ...
  return SimBriefApi.getSimBriefOfp(navigraphUsername, overrideSimBriefUserID)
    .then((data) => {
      ...
      mcdu.simbrief['blockFuel'] =
        mcdu.simbrief['units'] === 'kgs' ? data.fuel.plan_ramp : lbsToKg(data.fuel.plan_ramp);
      ...
      mcdu.simbrief['tripFuel'] =
        mcdu.simbrief['units'] === 'kgs' ? data.fuel.enroute_burn : lbsToKg(data.fuel.enroute_burn);
...
const insertUplink = (mcdu) => {
   ...
   */
  mcdu
    .setFromTo(originIcao, destinationIcao)
    .then(async (result) => {
      if (result) {
        if (originTransAlt > 0) {
  ...
  mcdu.updatePaxNo(paxCount, (result) => {
    if (result) {
      if (mcdu.page.Current === mcdu.page.InitPageA) {
        CDUInitPage.ShowPage1(mcdu);
      }
      ...
      getWaypointByIdentAndCoords(mcdu, routeIdent, coords, (waypoint) => {
        if (waypoint) {
          mcdu.flightPlanManager
            .addWaypoint(waypoint.icao, wpIndex, () => {
...
const insertCoRoute = async (mcdu) => {
  const { originIcao, destinationIcao, alternateIcao } = mcdu.coRoute;
