...
class A32NX_OneBrakeTemp {
  ...
  calculateDeltaCoolDown(deltaTemp, speed, deltaTempFactor) {
    return (
      deltaTemp *
      (BASE_SPEED + this.gearStatus.extended() * SPEED_COOLDOWN_FACTOR * speed) *
      BASE_COOL_DOWN_FACTOR *
      deltaTempFactor *
      this.brakeFan.fanMultiplier()
    );
  }
  ...
  update(deltaTime, ambientTemperature, airspeed) {
    ...
    if (Math.abs(deltaAmbient) > MIN_TEMP_DELTA) {
      const deltaTempFactor =
        1 + Math.pow(deltaAmbient, 2) * BASE_HEAT_DIFFERENTIAL_FACTOR * this.brakeFan.fanDifferentialFactor();
      const brakeCoolDown =
        secondRatio *
        getRandomArbitrary(0.8, 1.2) *
        this.calculateDeltaCoolDown(deltaAmbient, airspeed, deltaTempFactor);
    ...
    if (this.brakeFan.shouldBeOn()) {
      ...
      this.reportedBrakeTemp +=
        secondRatio * getRandomArbitrary(0.8, 1.2) * this.coolProbe(probeTargetTemp, this.reportedBrakeTemp);
    } else {
      // When fan is off, hot brakes will heat up the probe
      this.reportedBrakeTemp +=
        secondRatio * getRandomArbitrary(0.8, 1.2) * this.equalizeProbe(this.reportedBrakeTemp, this.brakeTemp);
    }
 ...
 * It owns one GearStatus per landing gear, a BrakeFan system and 8 brakes.
 */
class A32NX_BrakeTemp {
  constructor() {
    ...
    this.brakes = [
      new A32NX_OneBrakeTemp(
        0,
        'L:A32NX_BRAKE_TEMPERATURE_1',
      ...
      new A32NX_OneBrakeTemp(
        4,
        'L:A32NX_BRAKE_TEMPERATURE_5',
        'L:A32NX_REPORTED_BRAKE_TEMPERATURE_5',
        this.gearLeft,
        this.brakeFan,
      ),
      new A32NX_OneBrakeTemp(
        5,
        'L:A32NX_BRAKE_TEMPERATURE_6',
        'L:A32NX_REPORTED_BRAKE_TEMPERATURE_6',
        this.gearLeft,
        this.brakeFan,
      ),
      new A32NX_OneBrakeTemp(
        6,
        'L:A32NX_BRAKE_TEMPERATURE_7',
        'L:A32NX_REPORTED_BRAKE_TEMPERATURE_7',
        this.gearLeft,
        this.brakeFan,
      ),
      new A32NX_OneBrakeTemp(
        7,
        'L:A32NX_BRAKE_TEMPERATURE_8',
        'L:A32NX_REPORTED_BRAKE_TEMPERATURE_8',
        this.gearLeft,
        this.brakeFan,
      ),
    ];
