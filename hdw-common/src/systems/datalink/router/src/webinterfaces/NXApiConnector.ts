  ...
  public static async receiveMetar(icao: string, message: WeatherMessage): Promise<AtsuStatusCodes> {
    ...
    switch (storedMetarSrc) {
      ...
      case 'SAYINTENTIONS':
        ...
        return await SayIntentions.getMetar(body)
          .then((metar) => {
      ...
      default:
        return await Metar.get(icao, ConfigWeatherMap[storedMetarSrc])
          .then((data) => {
  ...
  public static async receiveAtis(icao: string, type: AtisType, message: AtisMessage): Promise<AtsuStatusCodes> {
    ...
    switch (storedAtisSrc) {
      case 'BEYONDATC':
        return await BeyondATC.getAtis(icao)
          .then((atis) => {
            ...
            message.Reports.push({ airport: icao, report: atis.toUpperCase() });
            return AtsuStatusCodes.Ok;
          })
          .catch(() => {
      ...
      case 'SAYINTENTIONS':
        ...
        return await SayIntentions.getAtis(body)
          .then((atis) => {
            if (!atis || atis === undefined || atis === '') {
              atis = 'D-ATIS NOT AVAILABLE';
            }
