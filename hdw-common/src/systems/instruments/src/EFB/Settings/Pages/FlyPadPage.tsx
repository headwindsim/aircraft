...
export const FlyPadPage = () => {
  ...
  const [theme, setTheme] = usePersistentProperty('EFB_UI_THEME', 'orange');
  ...
  const themeButtons: ButtonType[] = [
    { name: tt('Headwind.Settings.flyPad.Orange', language), setting: 'orange' },
  ...
  return (
    <SettingsPage name={tt('Settings.flyPad.Title', language)}>
      ...
      <SettingGroup>
        ...
        {!usingAutobrightness && (
          <SettingItem
            name={tt('Settings.flyPad.Brightness', language)}
            disabled={!!usingAutobrightness}
            groupType="parent"
          >
      ...
      <SettingItem name={tt('Settings.flyPad.ShowStatusBarFlightProgressIndicator', language)}>
        <Toggle
          value={!!showStatusBarFlightProgress}
          onToggle={(value) => setShowStatusBarFlightProgress(value ? 1 : 0)}
        />
      </SettingItem>
