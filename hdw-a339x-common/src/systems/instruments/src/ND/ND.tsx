...
import { EngModePage } from './pages/eng'
...
export class NDComponent<T extends number> extends DisplayComponent<NDProps<T>> {
  ...
  private readonly engPage = FSComponent.createRef<EngModePage<T>>();
  ...
  private handleNewMapPage(mode: EfisNdMode) {
    ...
    switch (mode) {
      ...
      case EfisNdMode.PLAN:
        ...
        break;
      case EfisNdMode.ENG:
        this.currentPageInstance = this.engPage.instance;
        break;
      default:
  ...
  render(): VNode | null {
    return (
      <>
        <div style={{ display: this.showOans.map((it) => (it && this.currentPageMode.get() !== EfisNdMode.ENG ? 'block' : 'none')) }}>
        ...
        <div style={{ display: this.showOans.map((it) => (it || this.currentPageMode.get() === EfisNdMode.ENG ? 'none' : 'block')) }}>
        ...
        <EngModePage backplane={this.props.backplane} ref={this.engPage} bus={this.props.bus} />
      </>
    );
  }
}
