...
export const ChartWidget: React.FC<ChartWidgetProps> = ({
  ...
  const draw = () => {
    ...
    const cgWeightToXY = (cg: number, weight: number): [number, number] => {
      ...
      const x =
        shiftX +
        xStart +
        (CanvasConst.yScale - y) * Math.tan((15 / 16) * Math.PI + (cg - limits.cg.min) * limits.cg.angleRad);
      return [x, y];
    };
  ...
  const mtow = {
    transform: `translateX(${(zfwCg < limits.cg.overlap ? limits.labels.mtow.x1 : limits.labels.mtow.x2) * width}px) translateY(${height * limits.labels.mtow.y}px)`,
  };
  const mlw = {
    transform: `translateX(${(zfwCg < limits.cg.overlap ? limits.labels.mlw.x1 : limits.labels.mlw.x2) * width}px) translateY(${height * limits.labels.mlw.y}px)`,
  };
  const mzfw = {
    transform: `translateX(${(zfwCg < limits.cg.overlap ? limits.labels.mzfw.x1 : limits.labels.mzfw.x2) * width}px) translateY(${height * limits.labels.mzfw.y}px)`,
  };
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < limits.cg.values.length; i++) {
      const newRow = {
        transform: `translateX(${(CanvasConst.cgAxis.xOffset + i * CanvasConst.cgAxis.xSpacing) * width}px) translateY(${height * CanvasConst.cgAxis.y}px)`,
      };
      cg.push(newRow);
    }
    setCgRows(cg);
