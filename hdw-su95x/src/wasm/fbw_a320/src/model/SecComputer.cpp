...
void SecComputer::step()
{
  ...
  if (SecComputer_U.in.sim_data.computer_running) {
    ...
    SecComputer_MATLABFunction_n(SecComputer_DWork.pRightStickDisabled &&
      ...
      &rtb_y_ev, &SecComputer_DWork.sf_MATLABFunction_g24);
    if (SecComputer_DWork.pLeftStickDisabled) {
      rtb_zeta_deg = SecComputer_P.Constant1_Value_p;
    } else {
      rtb_zeta_deg = SecComputer_U.in.analog_inputs.capt_roll_stick_pos;
    }

    if (!SecComputer_DWork.pRightStickDisabled) {
      rtb_Switch5 = SecComputer_U.in.analog_inputs.fo_roll_stick_pos;
    } else {
      rtb_Switch5 = SecComputer_P.Constant1_Value_p;
    }

    pair1SpdBrkCommand = rtb_Switch5 + rtb_zeta_deg;
    ...
    if (SecComputer_U.in.discrete_inputs.is_unit_1) {
      leftElevatorAvail = ((!SecComputer_U.in.discrete_inputs.l_elev_servo_failed) && rtb_y_j);
      rightElevatorAvail = ((!SecComputer_U.in.discrete_inputs.r_elev_servo_failed) && rtb_y_j);
    } else {
      leftElevatorAvail = ((!SecComputer_U.in.discrete_inputs.l_elev_servo_failed) && rtb_y_la);
    ...
    rtb_BusAssignment_n_logic_any_landing_gear_not_uplocked = rtb_AND4_a;
    ...
    if (rtb_doubleIrFault || ((SecComputer_B.in_flight != 0.0) &&
      ...
      rtb_pitchLawCapability = pitch_efcs_law::DirectLaw;
               ...
               SecComputer_DWork.abnormalConditionWasActive || ((!rtb_y_ls) && (!rtb_AND2_j) && ((!leftElevatorAvail) ||
      (!rightElevatorAvail)))) {
    ...
    if (!SecComputer_DWork.pRightStickDisabled) {
      rtb_Switch5 = SecComputer_U.in.analog_inputs.fo_pitch_stick_pos;
    } else {
      rtb_Switch5 = SecComputer_P.Constant_Value_p;
    }

    if (SecComputer_DWork.pLeftStickDisabled) {
      u0 = SecComputer_P.Constant_Value_p;
    } else {
      u0 = SecComputer_U.in.analog_inputs.capt_pitch_stick_pos;
    }

    u0 += rtb_Switch5;
    if (u0 > SecComputer_P.Saturation_UpperSat_d) {
      u0 = SecComputer_P.Saturation_UpperSat_d;
    } else if (u0 < SecComputer_P.Saturation_LowerSat_h) {
    ...
    if (SecComputer_DWork.Delay1_DSTATE_i) {
      ...
      rtb_Switch5 = SecComputer_P.Constant1_Value;
    } else {
      rtb_Switch5 = SecComputer_P.Constant2_Value;
    }

    SecComputer_RateLimiter(rtb_Switch5, SecComputer_P.RateLimiterVariableTs6_up,
    ...
    SecComputer_MATLABFunction(&SecComputer_U.in.bus_inputs.elac_1_bus.discrete_status_word_1,
      SecComputer_P.BitfromLabel2_bit_o, &rtb_y_p);
    SecComputer_MATLABFunction(&SecComputer_U.in.bus_inputs.elac_2_bus.discrete_status_word_1,
      SecComputer_P.BitfromLabel3_bit_j, &rtb_y_pa);
    ...
    if (SecComputer_U.in.discrete_inputs.is_unit_1) {
      ...
      pair2SpdBrkCommand = rtb_Switch5;
      ...
      pair1SpdBrkCommand = 0.0;
      pair2SpdBrkCommand = rtb_Switch5 / 2.0;
    }

    if (rtb_zeta_deg >= 0.0) {
      rtb_zeta_deg = pair1SpdBrkCommand - pair1RollCommand;
      pair1RollCommand = pair2SpdBrkCommand - rtb_Switch3;
    } else {
      rtb_zeta_deg = pair1SpdBrkCommand;
      pair1SpdBrkCommand += pair1RollCommand;
      pair1RollCommand = pair2SpdBrkCommand;
      pair2SpdBrkCommand += rtb_Switch3;
    }

    if (rtb_NOT_bl) {
      rtb_NOT_oi = rtb_handleIndex;
    } else {
      rtb_NOT_oi = std::fmax(rtb_zeta_deg - (pair1SpdBrkCommand - std::fmax(pair1SpdBrkCommand, -50.0)), -50.0);
    }

    if (rtb_NOT_oi > SecComputer_P.Saturation_UpperSat_n) {
      rtb_NOT_oi = SecComputer_P.Saturation_UpperSat_n;
    } else if (rtb_NOT_oi < SecComputer_P.Saturation_LowerSat_n) {
      rtb_NOT_oi = SecComputer_P.Saturation_LowerSat_n;
    }

    SecComputer_RateLimiter_b(rtb_NOT_oi, SecComputer_P.RateLimiterGenericVariableTs_up,
    ...
    if (rtb_NOT_bl) {
      ...
      rtb_NOT_oi = std::fmax(pair1SpdBrkCommand - (rtb_zeta_deg - std::fmax(rtb_zeta_deg, -50.0)), -50.0);
    }

    if (rtb_NOT_oi > SecComputer_P.Saturation1_UpperSat_e) {
      rtb_NOT_oi = SecComputer_P.Saturation1_UpperSat_e;
    } else if (rtb_NOT_oi < SecComputer_P.Saturation1_LowerSat_f) {
      rtb_NOT_oi = SecComputer_P.Saturation1_LowerSat_f;
    }

    SecComputer_RateLimiter_b(rtb_NOT_oi, SecComputer_P.RateLimiterGenericVariableTs1_up,
      SecComputer_P.RateLimiterGenericVariableTs1_lo, SecComputer_U.in.time.dt,
      SecComputer_U.in.analog_inputs.right_spoiler_1_pos_deg, !spoilerPair1SupplyAvail, &rtb_zeta_deg,
      &SecComputer_DWork.sf_RateLimiter_a);
    if (rtb_NOT_bl) {
      rtb_NOT_oi = rtb_handleIndex;
    } else {
      rtb_NOT_oi = std::fmax(pair1RollCommand - (pair2SpdBrkCommand - std::fmax(pair2SpdBrkCommand, -50.0)), -50.0);
    }

    if (rtb_NOT_oi > SecComputer_P.Saturation2_UpperSat) {
      rtb_NOT_oi = SecComputer_P.Saturation2_UpperSat;
    } else if (rtb_NOT_oi < SecComputer_P.Saturation2_LowerSat) {
      rtb_NOT_oi = SecComputer_P.Saturation2_LowerSat;
    }

    SecComputer_RateLimiter_b(rtb_NOT_oi, SecComputer_P.RateLimiterGenericVariableTs2_up,
    ...
    pair2SpdBrkCommand = rtb_zeta_deg;
    ...
    SecComputer_MATLABFunction(&SecComputer_U.in.bus_inputs.sfcc_1_bus.slat_flap_system_status_word,
      SecComputer_P.BitfromLabel_bit_a1, &rtb_y_pa);
    SecComputer_MATLABFunction(&SecComputer_U.in.bus_inputs.sfcc_1_bus.slat_flap_system_status_word,
      SecComputer_P.BitfromLabel1_bit_gf, &rtb_y_p);
    SecComputer_MATLABFunction(&SecComputer_U.in.bus_inputs.sfcc_1_bus.slat_flap_system_status_word,
    ...
    if (rtb_y_pa != 0U) {
      rtb_handleIndex = 0.0;
    } else if ((rtb_y_p != 0U) && (rtb_Switch7_c != 0U)) {
      rtb_handleIndex = 1.0;
    } else if ((rtb_y_p != 0U) && (rtb_Switch7_c == 0U)) {
      rtb_handleIndex = 2.0;
    } else if (rtb_y != 0U) {
    ...
    rtb_y_la = (rtb_OR1 || ((static_cast<real_T>(rtb_activePitchLaw) != SecComputer_P.CompareToConstant2_const_f) && (
    ...
    LawMDLOBJ2.step(&SecComputer_U.in.time.dt, &rtb_BusAssignment_f_logic_ir_computation_data_n_z_g,
      ...
      const_cast<real_T*>(&SecComputer_RGND)), (const_cast<real_T*>(&SecComputer_RGND)), &u0, &pair1SpdBrkCommand,
                    &rtb_y_la, &rtb_y_ei, &rtb_eta_deg, &rtb_eta_trim_dot_deg_s, &rtb_eta_trim_limit_lo,
    ...
    if (rtb_AND2_j) {
      rtb_zeta_deg = SecComputer_P.Gain_Gain_m * SecComputer_DWork.Delay_DSTATE;
      if (rtb_zeta_deg > SecComputer_P.Saturation_UpperSat) {
        rtb_zeta_deg = SecComputer_P.Saturation_UpperSat;
      } else if (rtb_zeta_deg < SecComputer_P.Saturation_LowerSat) {
        rtb_zeta_deg = SecComputer_P.Saturation_LowerSat;
      }
    } else if (SecComputer_U.in.discrete_inputs.ths_override_active) {
      rtb_zeta_deg = SecComputer_P.Constant_Value_a;
    } else {
      switch (static_cast<int32_T>(rtb_activePitchLaw)) {
       case 1:
       case 2:
        rtb_zeta_deg = rtb_eta_trim_dot_deg_s;
        break;
       ...
       default:
        rtb_zeta_deg = SecComputer_P.Constant1_Value_px;
        break;
      }
    }
    ...
    SecComputer_DWork.Delay_DSTATE = rtb_zeta_deg + SecComputer_DWork.Delay_DSTATE_l;
    ...
    if (SecComputer_U.in.discrete_inputs.is_unit_1) {
      rtb_y_la = SecComputer_U.in.discrete_inputs.pitch_not_avail_elac_1;
    } else {
      rtb_y_la = SecComputer_U.in.discrete_inputs.pitch_not_avail_elac_2;
    }

    rtb_NOT_bl = (rtb_y_la && (!rtb_isEngagedInPitch) && rtb_AND4_a);
    if (rtb_NOT_bl) {
    ...
    rtb_VectorConcatenate[15] = rtb_y_j;
    ...
    SecComputer_MATLABFunction_c(rtb_VectorConcatenate, &rtb_y_m);
    ...
    rtb_VectorConcatenate[5] = rtb_y_ev;
    ...
    SecComputer_Y.out.logic.right_sidestick_priority_locked = rtb_y_ev;
    SecComputer_Y.out.logic.total_sidestick_pitch_command = u0;
    SecComputer_Y.out.logic.ground_spoilers_armed = rtb_y_j;
    ...
    rtb_y_la = (rtb_isEngagedInPitch || rtb_NOT_bl);
    if (rtb_y_la && leftElevatorAvail) {
    ...
    if (rtb_y_la && rightElevatorAvail) {
    ...
    if (spoiler_pair_2_avail) {
      ...
      SecComputer_Y.out.analog_outputs.right_spoiler_2_pos_order_deg = rtb_NOT_oi;
    } else {
    ...
    SecComputer_Y.out.bus_outputs.discrete_status_word_1.SSM = static_cast<uint32_T>
      (SecComputer_P.EnumeratedConstant1_Value);
    SecComputer_Y.out.bus_outputs.discrete_status_word_1.Data = rtb_y_m;
    ...
    SecComputer_DWork.Delay1_DSTATE = rtb_y_ev;
