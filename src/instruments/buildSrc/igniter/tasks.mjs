...
export function getInstrumentsIgniterTasks() {
    ...
    return [
        ...baseInstruments.map(({ name }) => new ExecTask(
            ...
            [join('src/instruments/src', name), join('headwind-a330-900/html_ui/Pages/VCockpit/Instruments/A32NX_A339', name)],
        )),
        ...ecamPages.map(({ name, path }) => new ExecTask(
            name,
            `node src/instruments/buildSrc/igniter/worker.mjs ${name}`,
            [join('src/instruments/src', path), join('headwind-a330-900/html_ui/Pages/VCockpit/Instruments/A32NX_A339/EcamPages', name)],
        )),
    ];
}
