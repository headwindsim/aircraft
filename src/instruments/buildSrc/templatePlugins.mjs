...
export function getTemplatePlugin({ name, config, imports = [], isInstrument }) {
    if (isInstrument) {
        return instrumentTemplate({
            ...
            config,
            imports,
            outputDir: join(Directories.root, 'headwind-a330neo/html_ui/Pages/VCockpit/Instruments/A32NX_A339'),
        });
    }
    return ecamPageTemplate({
        name,
        outputDir: join(Directories.root, 'headwind-a330neo/html_ui/Pages/VCockpit/Instruments/A32NX_A339/EcamPages'),
    });
}
