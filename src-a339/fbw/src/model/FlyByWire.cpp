...
void FlyByWireModelClass::step()
{
  ...
  if (FlyByWire_DWork.sProtActive_c > FlyByWire_P.Switch_Threshold) {
  ...
  if (FlyByWire_DWork.is_active_c2_FlyByWire == 0U) {
    ...
    switch (FlyByWire_DWork.is_c2_FlyByWire) {
     ...
     case FlyByWire_IN_Flare_Set_Rate:
      ...
      if ((FlyByWire_U.in.data.H_radio_ft <= 40.0) || (rtb_Y_jv == 1.0)) {
  ...
  FlyByWire_RateLimiter(rtb_nz_limit_up_g, FlyByWire_P.RateLimiterVariableTs2_up_f,
                        ...
                        &FlyByWire_DWork.sf_RateLimiter_mr);
  ...
  FlyByWire_RateLimiter(static_cast<real_T>(rtb_in_flare), FlyByWire_P.RateLimiterVariableTs6_up,
                        FlyByWire_P.RateLimiterVariableTs6_lo, FlyByWire_U.in.time.dt,
                        FlyByWire_P.RateLimiterVariableTs6_InitialCondition, &rtb_Y_p2,
  ...
  if (FlyByWire_U.in.data.autopilot_custom_on > FlyByWire_P.Switch1_Threshold_ke) {
    ...
    if (rtb_Y_p2 > FlyByWire_P.Saturation_UpperSat) {
      rtb_Y_p2 = FlyByWire_P.Saturation_UpperSat;
    } else if (rtb_Y_p2 < FlyByWire_P.Saturation_LowerSat) {
      rtb_Y_p2 = FlyByWire_P.Saturation_LowerSat;
    }

    omega_0 = (FlyByWire_DWork.Delay_DSTATE_dq - rtb_GainTheta) * FlyByWire_P.Gain_Gain;
    if (omega_0 > FlyByWire_P.Saturation_UpperSat_j) {
      omega_0 = FlyByWire_P.Saturation_UpperSat_j;
    } else if (omega_0 < FlyByWire_P.Saturation_LowerSat_b) {
      omega_0 = FlyByWire_P.Saturation_LowerSat_b;
    }

    rtb_Y_g = (FlyByWire_P.Constant_Value_b - rtb_Y_p2) * FlyByWire_P.Constant_Value_m + omega_0 * rtb_Y_p2;
    ...
    if (FlyByWire_DWork.sProtActive > FlyByWire_P.Switch2_Threshold) {
      ...
      if (rtb_Y_mc5 > FlyByWire_P.Saturation8_UpperSat) {
        rtb_Y_p2 = FlyByWire_P.Saturation8_UpperSat;
      } else if (rtb_Y_mc5 < FlyByWire_P.Saturation8_LowerSat) {
        rtb_Y_p2 = FlyByWire_P.Saturation8_LowerSat;
      } else {
        rtb_Y_p2 = rtb_Y_mc5;
      }
      ...
      rtb_Y_p2 = FlyByWire_P.Constant1_Value;
    }

    rtb_Limiterxi2 = ((FlyByWire_P.Constant_Value_k - rtb_Saturation_kd) * rtb_Y_mc5 + rtb_Y_p2 * rtb_Saturation_kd) +
      rtb_Y_g;
  }
  ...
  FlyByWire_RateLimiter(static_cast<real_T>(rtb_nz_limit_lo_g) - std::fmin(5.0, std::fmax(0.0, 5.0 -
    (FlyByWire_U.in.data.V_ias_kn - (FlyByWire_U.in.data.VLS_kn + 5.0)) * 0.25)),
                        FlyByWire_P.RateLimiterVariableTs6_up_f, FlyByWire_P.RateLimiterVariableTs6_lo_m,
                        FlyByWire_U.in.time.dt, FlyByWire_P.RateLimiterVariableTs6_InitialCondition_b, &rtb_Y_g,
                        &FlyByWire_DWork.sf_RateLimiter);
  ...
  rtb_Y_p2 = FlyByWire_P.DiscreteDerivativeVariableTs2_Gain * FlyByWire_U.in.data.V_tas_kn;
  FlyByWire_LagFilter((rtb_Y_p2 - FlyByWire_DWork.Delay_DSTATE_o) / FlyByWire_U.in.time.dt, FlyByWire_P.LagFilter_C1_j,
                      FlyByWire_U.in.time.dt, &rtb_Y_p, &FlyByWire_DWork.sf_LagFilter_h);
  ...
  if (rtb_Y_p > FlyByWire_P.SaturationSpoilers_UpperSat) {
    rtb_Y_n0 = FlyByWire_P.SaturationSpoilers_UpperSat;
  } else if (rtb_Y_p < FlyByWire_P.SaturationSpoilers_LowerSat) {
    rtb_Y_n0 = FlyByWire_P.SaturationSpoilers_LowerSat;
  } else {
    rtb_Y_n0 = rtb_Y_p;
  }
  ...
  rtb_Sum1_h = (((rtb_Saturation_kd - FlyByWire_DWork.Delay_DSTATE_h) / FlyByWire_U.in.time.dt *
                 FlyByWire_P.Gain3_Gain_jk + rtb_Saturation3 * look1_binlxpw(FlyByWire_U.in.data.V_tas_kn,
    FlyByWire_P.PLUT_bp01Data_j, FlyByWire_P.PLUT_tableData_j, 1U)) + (rtb_Gain_ok - FlyByWire_DWork.Delay_DSTATE_dz) /
                FlyByWire_U.in.time.dt) + FlyByWire_P.Gain_Gain_f * rtb_Y_p;
  FlyByWire_WashoutFilter(std::fmin(FlyByWire_U.in.data.spoilers_left_pos, FlyByWire_U.in.data.spoilers_right_pos),
  ...
  FlyByWire_RateLimiter(rtb_BusAssignment_sim_input_delta_eta_pos, FlyByWire_P.RateLimiterVariableTs2_up_b,
                        ...
                        FlyByWire_P.RateLimiterVariableTs2_InitialCondition_j, &rtb_Y_cu,
                        &FlyByWire_DWork.sf_RateLimiter_p);
  rtb_y_l = (rtb_Y_k1 - rtb_Y_c) * rtb_Y_cu;
  ...
  rtb_Y_cu = FlyByWire_P.Subsystem1_Gain * rtb_Saturation3;
  rtb_Divide_k = (rtb_Y_cu - FlyByWire_DWork.Delay_DSTATE_ps) / FlyByWire_U.in.time.dt;
  ...
  if (omega_0 > FlyByWire_P.Saturation_UpperSat_j1) {
    omega_0 = FlyByWire_P.Saturation_UpperSat_j1;
  ...
  omega_0 = rtb_Y_n0 * look1_binlxpw(FlyByWire_U.in.data.H_radio_ft,
  ...
  FlyByWire_RateLimiter(rtb_uDLookupTable_g, FlyByWire_P.RateLimiterVariableTs_up_k,
                        ...
                        FlyByWire_P.RateLimiterVariableTs_InitialCondition_f, &rtb_Y_n0,
  ...
  } else if (FlyByWire_DWork.pY < FlyByWire_P.Saturation_LowerSat_bc) {
    rtb_Limiterxi1 = FlyByWire_P.Saturation_LowerSat_bc;
  } else {
  ...
  FlyByWire_Y.out.roll.data_computed.in_flight_gain = rtb_Y_n0;
  ...
  u0 = FlyByWire_P.Gainxi1_Gain_e * rtb_Y_d;
  ...
  FlyByWire_DWork.Delay_DSTATE_o = rtb_Y_p2;
  FlyByWire_DWork.Delay_DSTATE_h = rtb_Saturation_kd;
  FlyByWire_DWork.Delay_DSTATE_dz = rtb_Gain_ok;
  FlyByWire_DWork.Delay_DSTATE_bk = rtb_Gain_gh;
  FlyByWire_DWork.Delay_DSTATE_ps = rtb_Y_cu;
