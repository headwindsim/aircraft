...
export class NXDataStore {
    ...
    static get(key: string, defaultVal?: string): any {
        const val = GetStoredData(`A333X_${key}`);
    ...
    static set(key: string, val: string): void {
        SetStoredData(`A333X_${key}`, val);
        this.listener.triggerToAllSubscribers('A333X_NXDATASTORE_UPDATE', key, val);
    }

    static subscribe(key: string, callback: SubscribeCallback): SubscribeCancellation {
        return Coherent.on('A333X_NXDATASTORE_UPDATE', (updatedKey: string, value: string) => {
